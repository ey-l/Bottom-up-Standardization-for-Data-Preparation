...
class Lucid:
    def __init__(self, dataset: str, output_dir, AC: AtomCollection, scripts: List[LUCIDDAG], max_seq_len: int=2, log_config=None, verbose=True):
    ...
    def naive_exhaustive(self):
        """
        Wrapper for naive-exhaustive algorithm.
        """
        for i, current_dag in enumerate(self.scripts):
            self.script_i = i
            ...
            final_results = self.exhaustive_get_promissing(current_dag, results)
            
            # Write results
            filename = f"final_{filename}"
            write_to = os.path.join(str(self.output_dir), filename)
            write_result(correctness_filepath, final_results, write_to)
        if self.verbose:
            print("Done!")
    ...
    def exaustive_apply_steps(self, current_dag: LUCIDDAG):
        ...
        quality_score = self.compute_quality(current_dag)
        ...
        while i < self.max_seq_len:
            ...
            for current_dag in current_dags:
                ...
                for transformation in options: 
                    try:
                        ...
                        dag = transformation.apply(atom_collection=self.AC, verbose=self.verbose)
                        #self.csvlogger.info(["apply", time.time()-apply_start, "type", f"{transformation.type}"])
                        quality_score = self.compute_quality(dag)
                        current_result.setdefault(dag, {})
                        current_result[dag]['transformations'] = results[current_dag]['transformations']+[transformation]
                        current_result[dag]['quality_score'] = quality_score
            ...
            self.csvlogger.info(["_exhaustive_step", end, "seq_length", i, self.script_i])
            results |= current_result
            i += 1
    ...
    def exhaustive_get_promissing(self, current_dag: LUCIDDAG, results):
        ...
        for result in results:
            steps = results[result]["transformations"]
            print(steps)
            positions = self.exhaustive_search_position(current_dag, steps)
            print(positions)
            for pos in positions:
                tries += 1
                # A new DAG every possible pos combo
                ...
                try:
                    for step in steps:
                        if step.type == "insert_a_line":
                            step_with_position = Transformation(luciddag=copy.deepcopy(dag), type='insert_a_line', line=step.line, lineno=pos[i])
                            i += 1
                        transformations.append(step_with_position)
                        dag = step_with_position.apply(atom_collection=self.AC)
                    final_results.setdefault(dag, {})
                    final_results[dag]["transformations"] = transformations
                    final_results[dag]['quality_score'] = results[result]["quality_score"]
                    final_results[dag]['i'] = tries
                    try:
                        self.eval_data_correctness(dag, tries)
                    except Exception as ex:
                        semantic_errors += 1
                        print(ex)

                    # Some early stopping function
                    #if (tries - semantic_errors) > 10:
                    #    print(corr_fp)
                    #    return final_results
                except Exception as ex:
                    print("Error before eval_data_correctness:", ex)
        return final_results
        ...
        * (line: str, lineno) - insert a line in the LUCID graph at lineno 
        ...
        for line in self.AC.line_vocab:
            ...
            for i in range(len(current_lines)+1):
                ...
                options.append(Transformation(luciddag=dag, type='insert_a_line', line=line, lineno=i)) """
