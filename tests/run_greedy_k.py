...
def run_gkdiv(dataset:str, ac: AtomCollection, scripts: List[LUCIDDAG], log_dict: dict, seq_len: int, budget: int, semantic_on: bool, output_dir: Path):
...
def run_gk(dataset: str, ac: AtomCollection, scripts: List[LUCIDDAG], log_dict: dict, seq_len: int, budget: int, semantic_on: bool, output_dir: Path):
    # Standardize one script end-to-end
    for i, dag in enumerate(scripts):
        lucid = Lucid(dataset=dataset, output_dir=output_dir, AC=ac, scripts=scripts)
        log_dict["filepath"] = output_dir / f'runtime_{i}.csv'
            ...
            for budget in config_dict["budget"]:
                for semantic_on in config_dict["semantic_on"]:
                    ...
                    if config_dict["method"] == "lsd":
                        run_gkdiv(config_dict["dataset"], ac, scripts, log_dict, seq_len, budget, semantic_on, output_dir)
                    elif config_dict["method"] == "lsv":
                        run_gk(config_dict["dataset"], ac, scripts, log_dict, seq_len, budget, semantic_on, output_dir)
                    else:
