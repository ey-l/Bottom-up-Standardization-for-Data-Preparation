...
threshold_score = 0.7
threshold_feature = 5
_input1[['Cabin_1', 'Cabin_2', 'Cabin_3']] = _input1['Cabin'].str.split('/', 2, expand=True)
_input1['Whole_spent'] = _input1['RoomService'] + _input1['FoodCourt'] + _input1['ShoppingMall'] + _input1['Spa'] + _input1['VRDeck']
user_drop_cols = ['PassengerId', 'Name', 'VIP', 'Cabin']
_input1 = _input1.drop(user_drop_cols, axis=1)
df_copy = _input1.copy()
_input1.shape
plot_dataframe_structure(_input1)
plot_categorical(_input1)
duplicates(_input1)
(_input1, drop_cols) = drop_na(_input1, threshold_NaN)
dropped_cols = np.unique(np.concatenate((drop_cols, user_drop_cols)))
(_input1, encoded_cols) = encoding(_input1, threshold_cat, target_col)
visualizer = MissingValuesBar(features=_input1.select_dtypes(include=np.number).columns.tolist())