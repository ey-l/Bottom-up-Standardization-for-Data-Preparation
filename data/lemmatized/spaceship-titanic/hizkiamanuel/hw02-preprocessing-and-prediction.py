...
missingno.matrix(_input1, color=(0.06, 0.3, 0.57), fontsize=20)
missingno.matrix(_input0, color=(0.06, 0.3, 0.57), fontsize=20)
train_df_2 = _input1.drop(['Age', 'FoodCourt', 'ShoppingMall'], axis=1)
test_df_2 = _input0.drop(['Age', 'FoodCourt', 'ShoppingMall'], axis=1)
data_df = pd.concat([_input1, _input0], axis=0)
data_df_2 = pd.concat([train_df_2, test_df_2], axis=0)
data_df
data_df_2
data_df['HomePlanet'] = data_df['HomePlanet'].fillna('Unknown', inplace=False)
data_df_2['HomePlanet'] = data_df_2['HomePlanet'].fillna('Unknown', inplace=False)
data_df['CryoSleep'] = data_df['CryoSleep'].fillna('Unknown', inplace=False)
data_df_2['CryoSleep'] = data_df_2['CryoSleep'].fillna('Unknown', inplace=False)
data_df['Destination'] = data_df['Destination'].fillna('Unknown', inplace=False)
data_df_2['Destination'] = data_df_2['Destination'].fillna('Unknown', inplace=False)
data_df['Cabin'] = data_df['Cabin'].fillna('X', inplace=False)
data_df_2['Cabin'] = data_df_2['Cabin'].fillna('X', inplace=False)
...
data_oh = data_oh.drop(['Name', 'Cabin'], axis=1, inplace=False)
data_oh_2 = data_oh_2.drop(['Name', 'Cabin'], axis=1, inplace=False)
...
train_2 = data_std_2[0:len(_input1)]
test_2 = data_std_2[len(_input1):]
