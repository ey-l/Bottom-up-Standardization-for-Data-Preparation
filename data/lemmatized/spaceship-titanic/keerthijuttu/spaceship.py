...
trans_count
trans_percent = trans_count / len(_input1)
trans_percent
...
for col in combi:
    ...
    if combi[col].dtype == 'datetime64[ns]':
        combi[col] = combi[col].fillna(data, inplace=False)
combi
combi.isnull().sum()
...
combi['Room_Service_group'] = combi['Room_Service_group'].replace({'low': 1, 'mid': 2, 'high': 3}, inplace=False)
combi['Room_Service_group']
