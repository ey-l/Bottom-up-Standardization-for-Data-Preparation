...
print(f'The Shpae of the Train data : {_input1.shape}')
print(f'The Shpae of the Test data : {_input0.shape}')
_input1.head()
...
get_csv(_input1, 'Train')
get_csv(_input0, 'Test')
_input1.info()
_input1.describe().T
plt.figure(figsize=(22, 24))
plt.subplot(4, 3, 1)
_input1.dtypes.value_counts().plot(kind='pie', autopct='%.1f%%', colors=[my_colors[4], my_colors[5], my_colors[6]])
plt.subplot(4, 3, 2)
sns.countplot(data=_input1, x='Transported', palette=[my_colors[1], my_colors[2]])
print(clr.S + " --- Count of Nan's in Every Column --- " + clr.E)
_input1.isnull().sum()
plt.figure(figsize=(6, 4))
plt.title('Count of Nan in Every Columns')
_input1.isnull().sum().plot(kind='bar', color=my_colors[3])
print(clr.S + 'Percentage of Nan columns in Train data ' + clr.E)
round(_input1.isnull().sum() / len(_input1) * 100, 2)
num_data = _input1.select_dtypes(exclude=['object']).copy()
num_data.head()
plt.figure(figsize=(22, 24))
temp = num_data['Age']
...
cat_data = _input1.select_dtypes(exclude=['float']).copy()
cat_data.head()
cat_data.isnull().sum()
cat_data.nunique()
...
def preprocess(df, name):
    ...
    if name == 'Train':
        df = df.drop(df[df['RoomService'] > 10000].index, axis=0, inplace=False)
        df = df.drop(df[df['VRDeck'] > 20000].index, axis=0, inplace=False)
        df = df.drop(df[df['Spa'] > 20000].index, axis=0, inplace=False)
        df = df.drop(df[df['FoodCourt'] > 20000].index, axis=0, inplace=False)
        df = df.drop(df[df['ShoppingMall'] > 20000].index, axis=0, inplace=False)
    return df
_input1 = preprocess(_input1, 'Train')
_input0 = preprocess(_input0, 'Test')
print(clr.S + 'Missing Train Data ' + clr.E, _input1.isnull().sum().sum())
print(clr.S + 'Missing Test Data ' + clr.E, _input0.isnull().sum().sum())
    ...
    for i in range(len(correlation.columns)):
        for j in range(i):
            if abs(correlation.iloc[i, j]) > thresh:
                ...
                corr_col.add(column)
    return corr_col
correlated_features = remove_feature(_input1, 0.8)
print(f'Highly correlated Features : {correlated_features}')
_input1 = _input1.drop(['PassengerId', 'Cabin', 'group_id', 'Deck', 'Age'], axis=1, inplace=False)
_input0 = _input0.drop(['PassengerId', 'Cabin', 'group_id', 'Deck', 'Age'], axis=1, inplace=False)
_input1.head()
