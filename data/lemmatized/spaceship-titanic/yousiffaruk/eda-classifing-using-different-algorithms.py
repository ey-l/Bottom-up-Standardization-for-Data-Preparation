...
def nom(name):
    for i in name:
        print('No. of {} = {}'.format(i, _input1[i].nunique()))
nom(['Cabin', 'Destination', 'Name', 'PassengerId', 'HomePlanet'])
_input1 = _input1.drop(columns=['PassengerId', 'Name', 'Cabin'], inplace=False)
_input1.columns
sns.countplot(data=_input1, y='HomePlanet', hue='Destination')
sns.catplot(x='HomePlanet', hue='CryoSleep', col='Destination', kind='count', data=_input1)
sns.catplot(x='HomePlanet', hue='VIP', col='Destination', kind='count', data=_input1)
plt.figure(figsize=(10, 7))
cor_matrix = _input1.loc[:, 'RoomService':'VRDeck']
sns.heatmap(cor_matrix.corr(), annot=True)
sns.pairplot(_input1.loc[:, 'VIP':'VRDeck'], hue='VIP', palette='muted')
from sklearn.preprocessing import OneHotEncoder
from sklearn.preprocessing import StandardScaler
from sklearn.compose import ColumnTransformer
from sklearn.model_selection import train_test_split
label = _input1['Transported']
train_1 = _input1.drop('Transported', axis=1)
