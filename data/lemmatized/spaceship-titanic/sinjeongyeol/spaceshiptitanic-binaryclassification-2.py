...
dataset = pd.concat([_input1, _input0], axis=0).reset_index(drop=True)
(dataset.shape, _input1.shape, _input0.shape)
...
mean_cnt = mean_cnt.drop(['mean', 'count'], axis=1, inplace=False)
mean_cnt
...
def get_oof(name, model):
    ...
    test_predict = np.zeros((len(_input0), 5))
    print('TRAIN ', name)
