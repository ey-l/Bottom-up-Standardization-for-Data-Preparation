...
_input2
_input1.isnull().sum()
train1 = _input1.dropna()
train1
train1['Transported'] = train1['Transported'].replace({False: 0, True: 1}, inplace=False)
train1['Transported']
...
ax = plt.subplot()
ax.scatter(train1[train1['Transported'] == 1]['Age'], train1[_input1['Transported'] == 1]['FoodCourt'], c='green', s=train1[train1['Transported'] == 1]['VRDeck'])
ax.scatter(train1[train1['Transported'] == 0]['Age'], train1[_input1['Transported'] == 0]['FoodCourt'], c='red', s=train1[train1['Transported'] == 0]['VRDeck'])
target = train1['Transported']
train1 = train1.drop(['Transported'], axis=1, inplace=False)
train1
combi = train1.append(_input0)
combi
combi.info()
combi.describe()
combi.isnull().sum()
...
for col in combi:
    if combi[col].dtype == 'object':
        combi[col] = combi[col].fillna('not listed', inplace=False)
    if combi[col].dtype == 'int':
        combi[col] = combi[col].fillna(combi[col].mean(), inplace=False)
    if combi[col].dtype == 'float':
        combi[col] = imp.fit_transform(combi[col].values.reshape(-1, 1))
    if combi[col].dtype == 'datetime64[ns]':
        combi[col] = combi[col].fillna(date, inplace=False)
combi
combi.isnull().sum()
...
combi['HomePlanet'] = combi['HomePlanet'].replace({'Earth': 1, 'Europa': 2, 'Mars': 3, 'not listed': 4}, inplace=False)
combi['HomePlanet']
...
combi['Destination'] = combi['Destination'].replace({'TRAPPIST-1e': 1, '55 Cancri e': 2, 'PSO J318.5-22': 3, 'not listed': 4}, inplace=False)
combi['Destination']
...
combi['Age_group'] = combi['Age_group'].replace({'young adult': 1, 'child': 2, 'middle age': 3, 'pensioner': 4}, inplace=False)
combi['Age_group']
...
combi['Room_Service_group'] = combi['Room_Service_group'].replace({'low': 1, 'med': 2, 'high': 3}, inplace=False)
combi['Room_Service_group']
...
combi['Food_Court_group'] = combi['Food_Court_group'].replace({'low': 1, 'med': 2, 'high': 3}, inplace=False)
combi['Food_Court_group']
...
combi['Shopping_group'] = combi['Shopping_group'].replace({'low': 1, 'med': 2, 'high': 3}, inplace=False)
combi['Shopping_group']
...
combi['Spa_group'] = combi['Spa_group'].replace({'low': 1, 'med': 2, 'high': 3}, inplace=False)
combi['Spa_group']
...
combi['VR_group'] = combi['VR_group'].replace({'low': 1, 'med': 2, 'high': 3}, inplace=False)
combi['VR_group']
combi.info()
