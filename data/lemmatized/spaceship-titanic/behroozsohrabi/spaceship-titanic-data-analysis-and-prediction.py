...
print('Train Dataset Shape:', _input1.shape)
print('Test Dataset Shape:', _input0.shape)
_input1.head(5)
df = pd.concat([_input1, _input0])
df = df.reset_index(drop=True, inplace=False)
print('# of Null values in Transported column:\t\t\t', df['Transported'].isnull().sum(), '\nMust be equal to # of rows in the test dataframe:\t', _input0.shape[0])
df['Destination'].unique()
df = df.replace({'Destination': {'TRAPPIST-1e': 'A', 'PSO J318.5-22': 'B', '55 Cancri e': 'C'}}, inplace=False)
...
df = df.replace(['None', 'nan'], np.nan, inplace=False)
