...
_input1.info()
_input1.isnull().sum()
categorical_features = _input1.select_dtypes('object').columns.to_list()
print(categorical_features)
numerical_features = _input1.drop(['Transported'], axis=1).select_dtypes(np.number).columns.to_list()
print(numerical_features)
spend_feature_list = _input1.drop(['Transported', 'Age'], axis=1).select_dtypes(np.number).columns.to_list()
for col in spend_feature_list:
    _input1[col] = _input1[col].fillna(0)
    _input0[col] = _input0[col].fillna(0)
from sklearn.impute import KNNImputer
train_data_num = _input1[numerical_features]
numerical_imputer = KNNImputer(n_neighbors=2)
train_num_transformed = numerical_imputer.fit_transform(train_data_num)
train_num_df = pd.DataFrame(train_num_transformed, columns=numerical_features, index=_input1.index)
print(train_num_df.head())
print('------------------')
print('Missing values:')
print(train_num_df.isnull().sum())
test_data_num = _input0[numerical_features]
test_num_transformed = numerical_imputer.fit_transform(test_data_num)
test_num_df = pd.DataFrame(test_num_transformed, columns=numerical_features, index=_input0.index)
print(test_num_df.head())
print('------------------')
print('Missing values:')
print(test_num_df.isnull().sum())
target = _input1['Transported']
train_cat_df = _input1[categorical_features]
train_df = pd.concat([train_cat_df, train_num_df], axis=1)
train_df['Transported'] = target
train_df
test_cat_df = _input0[categorical_features]
