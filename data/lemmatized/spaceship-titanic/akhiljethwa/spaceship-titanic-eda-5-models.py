...
_input0.info()
print('-------' * 6)
train = _input1.drop(['Name', 'PassengerId', 'Cabin'], axis=1)
test = _input0.drop(['Name', 'PassengerId', 'Cabin'], axis=1)
train.head()
test.head()
...
for col in LABELS:
    if col in ['Age', 'RoomService', 'FoodCourt', 'ShoppingMall', 'Spa', 'VRDeck']:
        train[col] = train[col].fillna(train[col].median(), inplace=False)
    else:
        train[col] = train[col].fillna(train[col].mode()[0], inplace=False)
train.head()
nan_cols = train.columns.tolist()
plt.figure(figsize=(16, 8))
nan_count_cols = train[nan_cols].isna().sum()
...
for col in LABELS:
    if col in ['Age', 'RoomService', 'FoodCourt', 'ShoppingMall', 'Spa', 'VRDeck']:
        test[col] = test[col].fillna(test[col].median(), inplace=False)
    else:
        test[col] = test[col].fillna(test[col].mode()[0], inplace=False)
train.head()
nan_cols = test.columns.tolist()
plt.figure(figsize=(16, 8))
nan_count_cols = test[nan_cols].isna().sum()
