...
for col in ['RoomService', 'FoodCourt', 'ShoppingMall', 'Spa', 'VRDeck', 'Age']:
    train_data[col] = train_data[col].fillna(train_data[col].median(), inplace=False)
    test_data[col] = test_data[col].fillna(train_data[col].median(), inplace=False)
test_data['VIP'] = test_data['VIP'].fillna(False, inplace=False)
train_data['VIP'] = train_data['VIP'].fillna(False, inplace=False)
print(train_data.isna().sum())
print('Missing data %: ', train_data.isna().sum().sum() / train_data.shape[0])
train_data['Name'] = train_data['Name'].fillna('', inplace=False)
(train_data.shape, train_data.isna().sum().sum())
train_data['Cabin'] = train_data['Cabin'].fillna('Z/0000/Z', inplace=False)
test_data['Cabin'] = test_data['Cabin'].fillna('Z/0000/Z', inplace=False)
...
train_data = train_data.drop('all', axis=1, inplace=False)
