...
_input1.describe().T
_input0.describe().T
df1 = _input1.drop(['PassengerId', 'Cabin', 'Name'], axis=1)
df1
df2 = _input0.drop(['PassengerId', 'Cabin', 'Name'], axis=1)
df2
df1.isna().sum()
df2.isna().sum()
LABELS = df1.columns
for col in LABELS:
    if col in ['Age', 'RoomService', 'FoodCourt', 'ShoppingMall', 'Spa', 'VRDeck']:
        df1[col] = df1[col].fillna(df1[col].median(), inplace=False)
    else:
...
for col in LABELS2:
    if col in ['Age', 'RoomService', 'FoodCourt', 'ShoppingMall', 'Spa', 'VRDeck']:
        df2[col] = df2[col].fillna(df2[col].median(), inplace=False)
    else:
        df2[col] = df2[col].fillna(df2[col].mode()[0], inplace=False)
df2.head()
df2.isna().sum()
