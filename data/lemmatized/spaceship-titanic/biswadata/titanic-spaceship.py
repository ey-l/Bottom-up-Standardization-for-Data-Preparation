...
_input1.describe().style.background_gradient()
Transported_data = _input1.Transported
df_combined = pd.concat([_input1.drop('Transported', axis=1), _input0], axis=0)
...
for i in categorical:
    if i != 'CryoSleep':
        ...
        y = x.to_string()
        y_converted_to_string = y.strip('0 ')
        df_combined1[i] = df_combined1[i].fillna(y_converted_to_string, inplace=False)
for i in numerical:
...
df_combined1.CryoSleep = df_combined1.CryoSleep.fillna(bool(0), inplace=False)
df_combined1.isnull().sum()
...
df_combined1 = df_combined1.drop(['Name', 'Cabin', 'PassengerId'], axis=1, inplace=False)
df_combined1.head(2)
