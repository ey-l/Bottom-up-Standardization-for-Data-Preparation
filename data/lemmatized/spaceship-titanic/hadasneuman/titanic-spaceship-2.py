...
_input1.describe()
_input1.info()
_input1.groupby(['Transported']).mean()
import matplotlib.pyplot as plt
import seaborn as sns
corrmat = _input1.corr()
(f, ax) = plt.subplots(figsize=(12, 9))
sns.heatmap(corrmat, vmax=0.8, square=True)
_input1.boxplot(column=['Age'], by='Transported')
_input1.boxplot(column=['Spa'], by='Transported')
_input1.boxplot(column=['RoomService'], by='Transported')
_input1.boxplot(column=['VRDeck'], by='Transported')
X = _input1.copy()
X_test = _input0.copy()
y = X['Transported']
X = X.drop(['Transported'], axis=1, inplace=False)
all_dfs = [X, X_test]
X_test.head()
X.isnull().sum()
...
for df in all_dfs:
    ...
    df = df.drop(['PassengerId', 'Group'], axis=1, inplace=False)
X_imp.head()
