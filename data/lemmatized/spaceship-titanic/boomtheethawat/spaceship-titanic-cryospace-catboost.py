...
_input1.head()
_input0.head()
_input1.describe(percentiles=[i / 10 for i in range(1, 10)])
corr = _input1.corr()
heatmap = sb.heatmap(corr, cmap='summer_r', annot=True)
(f, (ax1, ax2)) = plt.subplots(nrows=2, ncols=3, figsize=(20, 10))
sb.boxplot(x=_input1['Age'], hue='Transported', data=_input1, ax=ax1[0])
sb.boxplot(x=_input1['RoomService'], hue='Transported', data=_input1, ax=ax1[1])
sb.boxplot(x=_input1['FoodCourt'], hue='Transported', data=_input1, ax=ax1[2])
sb.boxplot(x=_input1['ShoppingMall'], hue='Transported', data=_input1, ax=ax2[0])
sb.boxplot(x=_input1['Spa'], hue='Transported', data=_input1, ax=ax2[1])
sb.boxplot(x=_input1['VRDeck'], hue='Transported', data=_input1, ax=ax2[2])
(f, (ax1, ax2)) = plt.subplots(nrows=2, ncols=3, figsize=(20, 10))
sb.kdeplot(x=_input1['Age'], hue='Transported', data=_input1, ax=ax1[0])
sb.kdeplot(x=_input1['RoomService'], hue='Transported', data=_input1, ax=ax1[1])
sb.kdeplot(x=_input1['FoodCourt'], hue='Transported', data=_input1, ax=ax1[2])
sb.kdeplot(x=_input1['ShoppingMall'], hue='Transported', data=_input1, ax=ax2[0])
sb.kdeplot(x=_input1['Spa'], hue='Transported', data=_input1, ax=ax2[1])
sb.kdeplot(x=_input1['VRDeck'], hue='Transported', data=_input1, ax=ax2[2])
_input1[['Cabin1', 'Cabin2', 'Cabin3']] = _input1['Cabin'].str.split('/', expand=True)
_input1[['Id', 'GroupSize']] = _input1['PassengerId'].str.split('_', expand=True)
df = _input1.drop(columns=['Name', 'PassengerId', 'VIP', 'Cabin', 'Cabin2', 'Id'])
_input0[['Cabin1', 'Cabin2', 'Cabin3']] = _input0['Cabin'].str.split('/', expand=True)
_input0[['Id', 'GroupSize']] = _input0['PassengerId'].str.split('_', expand=True)
test_df = _input0.drop(columns=['Name', 'PassengerId', 'VIP', 'Cabin', 'Cabin2', 'Id'])
sb.set_palette('pastel')
sb.set()
(f, (ax1, ax2)) = plt.subplots(nrows=2, ncols=3, figsize=(20, 10))
sb.countplot(data=_input1, x='HomePlanet', hue='Transported', ax=ax1[0], palette=['r', 'g'])
sb.countplot(data=_input1, x='Destination', hue='Transported', ax=ax1[1], palette=['r', 'g'])
sb.countplot(data=_input1, x='CryoSleep', hue='Transported', ax=ax1[2], palette=['r', 'g'])
sb.countplot(data=_input1, x='VIP', hue='Transported', ax=ax2[0], palette=['r', 'g'])
sb.countplot(data=_input1, x='Cabin1', hue='Transported', ax=ax2[1], palette=['r', 'g'])
sb.countplot(data=_input1, x='Cabin3', hue='Transported', ax=ax2[2], palette=['r', 'g'])
