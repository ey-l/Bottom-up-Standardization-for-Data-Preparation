...
_input1.Transported = _input1.Transported.astype('float')
_input0['Transported'] = 0.5
dataset = pd.concat([_input1, _input0], axis=0)
dataset = dataset.reset_index(inplace=False, drop=True)
dataset = dataset.sort_values(by=['PassengerId'], inplace=False)
...
tmp = dataset.Age < 13
dataset.RoomService[tmp] = dataset.RoomService[tmp].fillna(0, inplace=False)
dataset.FoodCourt[tmp] = dataset.FoodCourt[tmp].fillna(0, inplace=False)
dataset.ShoppingMall[tmp] = dataset.ShoppingMall[tmp].fillna(0, inplace=False)
dataset.Spa[tmp] = dataset.Spa[tmp].fillna(0, inplace=False)
dataset.VRDeck[tmp] = dataset.VRDeck[tmp].fillna(0, inplace=False)
...
dataset.RoomService = dataset.RoomService.fillna(0, inplace=False)
dataset.FoodCourt = dataset.FoodCourt.fillna(0, inplace=False)
dataset.ShoppingMall = dataset.ShoppingMall.fillna(0, inplace=False)
dataset.Spa = dataset.Spa.fillna(0, inplace=False)
dataset.VRDeck = dataset.VRDeck.fillna(0, inplace=False)
...
GroupID = GroupID.replace(groups.HomePlanet.index, groups.HomePlanet.values, inplace=False)
tmp = pd.isna(dataset.HomePlanet) & (dataset.inGroup == 1)
dataset.HomePlanet[tmp] = GroupID[tmp]
print('Fixed HomePlanet:     ', tmp.sum())
GroupID = dataset.GroupID.copy()
GroupID = GroupID.replace(groups.Cabin.index, groups.Cabin.values, inplace=False)
tmp = pd.isna(dataset.Cabin) & (dataset.inGroup == 1)
dataset.Cabin[tmp] = GroupID[tmp]
print('Fixed Cabin:          ', tmp.sum())
GroupID = dataset.GroupID.copy()
GroupID = GroupID.replace(groups.Destination.index, groups.Destination.values, inplace=False)
