...
data_dropped = _input1.dropna()
print(len(data_dropped))
print(1 - len(data_dropped) / len(_input1))
test_size = 0.2
(train, test) = model_selection.train_test_split(_input1, test_size=test_size, random_state=19)
...
def cutter(col, bins):
    ...
    def cut(df):
        _input1 = pd.cut(df[col], bins=bins, labels=range(len(bins) - 1))
        return _input1.to_frame(f'{col}Threshold').astype(float)
    return cut
