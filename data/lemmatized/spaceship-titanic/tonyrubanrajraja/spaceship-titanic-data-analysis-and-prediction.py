...
_input1.info()
_input1.head()
plt.bar(_input1.columns, _input1.isna().sum())
plt.xticks(rotation=90)
plt.figure(figsize=(15, 10))
sns.heatmap(_input1.loc[:, ~_input1.columns.isin(['PassengerId', 'Transported'])].isna(), cmap='YlGnBu')
msno.heatmap(_input1, figsize=(15, 10))
...
for i in range(0, len(numerical_cols)):
    ...
    plt.hist(_input1.loc[:, numerical_cols[i]])
...
(train_df, y, imputers, encoders) = preprocess_training_data(_input1)
train_df.head()
train_df.isna().sum()
...
test_df = preprocess_test_data(_input0, imputers, encoders)
test_df.head()
test_df.isna().sum()
