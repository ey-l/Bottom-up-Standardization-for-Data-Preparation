...
print(f'Train dataframe has {_input1.shape[0]} training examples and {_input1.shape[1]} features')
print(f'Test dataframe has {_input0.shape[0]} Testing examples and {_input0.shape[1]} features')
...
Null_test(_input1, plot=True)
Null_test(_input0, plot=True)
...
def FeatureEngineering(dataframe):
    ...
    df['CabinSide'] = df['CabinSide'].fillna('P', inplace=False)
    df['CabinDeck'] = df['CabinDeck'].fillna('T', inplace=False)
    ...
    df = df.replace({'Transported': {False: 0, True: 1}}, inplace=False)
    for col in Expenses:
        df[col] = df[col].fillna(0, inplace=False)
    df['TotalCosts'] = df[Expenses].sum(axis=1)
    df['Age'] = df['Age'].fillna(df['Age'].mean(), inplace=False)
    df['CabinNum'] = df['CabinNum'].fillna(df['CabinNum'].mean(), inplace=False)
    df = df.drop(['Cabin', 'Name'], axis=1, inplace=False)
    return df
train_df_new = FeatureEngineering(_input1)
test_df_new = FeatureEngineering(_input0)
print('Train dataframe information')
train_df_new.info()
print('\n Test dataframe information')
test_df_new.info()
