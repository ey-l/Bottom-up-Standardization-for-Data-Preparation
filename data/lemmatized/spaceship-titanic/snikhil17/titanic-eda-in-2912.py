...
_input1.info()
_input1.shape
missing_val_df = (_input1.isnull().sum() / _input1.shape[0] * 100).to_frame().reset_index().rename({'index': 'columns', 0: 'Missing Values percentage'}, axis=1)
...
missing_val_df_test = (_input0.isnull().sum() / _input0.shape[0] * 100).to_frame().reset_index().rename({'index': 'columns', 0: 'Missing Values percentage'}, axis=1)
plt.figure(figsize=(10, 8))
...
plt.yticks(fontsize=12, fontweight='bold', color='maroon')
plt.figure(figsize=(10, 8))
sns.countplot(data=_input1, x='Transported', palette='dark')
...
_input1.info()
cat_cols = [col for col in _input1.columns if _input1[col].dtype == 'object']
for col in cat_cols:
    print(f'Number of Rows: {_input1.shape[0]}')
    print(col)
    print(f'First 5 Unique Values: {_input1[col].unique()[:5]}')
    print(f'Number of Unique Values: {_input1[col].nunique()}')
    print(f'Value Counts: \n{_input1[col].value_counts(dropna=False).head(5)}')
    print('=' * 50)
_input1[['CryoSleep', 'Destination', 'VIP']].groupby(['Destination', 'VIP'])[['CryoSleep']].agg(['count', 'mean', 'median', pd.Series.mode])
_input1['isTrain'] = True
_input0['isTrain'] = False
tt = pd.concat([_input1, _input0]).reset_index(drop=True).copy()
tt.head()
