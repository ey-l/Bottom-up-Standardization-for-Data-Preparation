...
_input1 = pd.read_csv('data/input/spaceship-titanic/train.csv', encoding='cp949', thousands=',')
tempDf = _input1
_input1.info()
_input1.head()
_input1.describe(include='all')
df_titanic_col = list(_input1.columns)
pd.DataFrame(df_titanic_col).T
df = pd.DataFrame()
df['Group'] = _input1['PassengerId'].apply(lambda x: x[:4]).astype(int)
df['GNum'] = _input1['PassengerId'].apply(lambda x: x[5:]).astype(int)
df = pd.concat([df, _input1[df_titanic_col[1:3]]], axis=1)
df['Deck'] = _input1['Cabin'].apply(lambda x: x.split('/')[0] if type(x) != float else None)
df['DNum'] = _input1['Cabin'].apply(lambda x: x.split('/')[1] if type(x) != float else None)
df['Side'] = _input1['Cabin'].apply(lambda x: x.split('/')[2] if type(x) != float else None)
df = pd.concat([df, _input1[df_titanic_col[4:12]]], axis=1)
df = pd.concat([df, _input1[df_titanic_col[-1]]], axis=1)
