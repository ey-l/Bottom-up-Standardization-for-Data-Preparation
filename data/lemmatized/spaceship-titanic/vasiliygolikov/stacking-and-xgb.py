...
_input1.head()
_input0.head()
(_input1.info(), _input0.info())
_input1 = _input1.drop(['Name'], axis=1)
_input0 = _input0.drop(['Name'], axis=1)
train_groups = np.array(list(map(lambda x: int(x.split('_')[0]), _input1.PassengerId)))
train_groups
test_groups = np.array(list(map(lambda x: int(x.split('_')[0]), _input0.PassengerId)))
test_groups
len(set(test_groups) & set(train_groups))
(np.mean(_input1.Transported == 1), np.mean(_input1.Transported == 0))
_input1['Transported'] = _input1['Transported'].astype(int)
_input1.info()
coorelation_matrix = np.corrcoef(_input1.dropna()[['RoomService', 'FoodCourt', 'ShoppingMall', 'Spa', 'VRDeck', 'Transported']], rowvar=0)
coorelation_matrix
corr = _input1.dropna()[['RoomService', 'FoodCourt', 'ShoppingMall', 'Spa', 'VRDeck', 'Transported']].corr()
corr.style.background_gradient(cmap='coolwarm')
fig = plt.figure()
axes = [fig.add_subplot(311), fig.add_subplot(312), fig.add_subplot(313)]
cols = ['RoomService', 'Spa', 'VRDeck']
for i in range(3):
    axes[i].hist(_input1[cols[i]][_input1.Transported == 1], color='green', alpha=0.5, label='Transported')
    axes[i].hist(_input1[cols[i]][_input1.Transported == 0], color='red', alpha=0.5, bins=50, label='Not transported')
...
for i in range(2):
    axes[i].hist(_input1[cols[i]][_input1.Transported == 1], color='green', alpha=0.5, bins=70, label='Transported')
    axes[i].hist(_input1[cols[i]][_input1.Transported == 0], color='red', alpha=0.5, bins=50, label='Not transported')
    axes[i].set_xlabel(cols[i])
    axes[i].legend()
fig.set_figheight(8)
...
plt.subplots_adjust(wspace=0.5, hspace=0.6)
plt.close()
(_input1.Cabin.unique().shape[0], _input1.shape[0])
...
def deck_side_encode(df):
    ...
    return new_df
dse = deck_side_encode(_input1)
dse.head()
_input1.HomePlanet.unique()
_input1.Destination.unique()
pd.get_dummies(_input1, dummy_na=True, columns=['HomePlanet', 'Destination'])
train_enc = pd.get_dummies(deck_side_encode(_input1), dummy_na=True, columns=['HomePlanet', 'Destination', 'Side', 'Deck'])
train_enc.head()
_input1 = pd.read_csv('data/input/spaceship-titanic/train.csv')
_input0 = pd.read_csv('data/input/spaceship-titanic/test.csv')
...
def data_prep(tr, te, drop=None):
    ...
    return (tr_1, te_1)
(train_prep, test_prep) = data_prep(_input1, _input0)
