...
print('train : ' + get_null_row_ratio(_input1))
print('test : ' + get_null_row_ratio(_input0))
...
plot_null_value_count(_input1, _input0)
...
def plot_null_value_ratio(df1, df2):
    ...
    (f, ax) = plt.subplots(figsize=(5, 5))
    sns.barplot(data=null_values, y='column', x='null count', hue='data', orient='h')
plot_null_value_ratio(_input1, _input0)
...
def split_id(df):
    ...
    temp_df = temp_df.drop(['PassengerId'], axis=1, inplace=False)
    return temp_df
...
def split_name(df):
    ...
    temp_df = temp_df.drop(['Name'], axis=1, inplace=False)
    return temp_df
...
def split_cabin(df):
    ...
    temp_df = temp_df.drop(['Cabin'], axis=1, inplace=False)
    return temp_df
...
train_test = pre_process_df(_input1)
test_test = pre_process_df(_input0)
