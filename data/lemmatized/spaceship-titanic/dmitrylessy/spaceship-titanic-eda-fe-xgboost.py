...
pandas_profiling.ProfileReport(_input0)
cat_cols = ['HomePlanet', 'CryoSleep', 'Destination', 'VIP']
(fig, axes) = plt.subplots(4, 1, figsize=(15, 30))
for (i, col) in enumerate(cat_cols):
    axis = axes[i]
    col_data_train = _input1[col].value_counts().to_frame() / len(_input1)
    col_data_train['Set'] = 'Train'
    col_data_test = _input0[col].value_counts().to_frame() / len(_input0)
...
for (i, col) in enumerate(cat_cols):
    axis = axes[i]
    col_data_trans = _input1[col][Y_train == 0].value_counts().to_frame()
    col_data_trans['Target'] = 'Transpoted'
    col_data_not = _input1[col][Y_train == 1].value_counts().to_frame()
...
for (i, col) in enumerate(num_cols[:]):
    axis = axes[i]
    log_scale = (i > 0) * 10
    col_data = pd.concat([_input1[col], _input0[col]], axis=1, ignore_index=True)
    if log_scale:
        col_data = col_data.replace({0: np.nan}, inplace=False)
...
X_train = X_train.drop(['Name'], axis=1, inplace=False)
X_test = X_test.drop(['Name'], axis=1, inplace=False)
...
X_train['SummaryExpenses'] = X_train['SummaryExpenses'].fillna(0, inplace=False)
X_test['SummaryExpenses'] = X_test['SummaryExpenses'].fillna(0, inplace=False)
X_train['VIP'] = X_train['VIP'].fillna(False, inplace=False)
X_test['VIP'] = X_test['VIP'].fillna(False, inplace=False)
X_train['CryoSleep'] = X_train['CryoSleep'].fillna(X_train['SummaryExpenses'] == 0, inplace=False)
X_test['CryoSleep'] = X_test['CryoSleep'].fillna(X_test['SummaryExpenses'] == 0, inplace=False)
mean = X_train['Age'].mean()
X_train['Age'] = X_train['Age'].fillna(mean, inplace=False)
X_test['Age'] = X_test['Age'].fillna(mean, inplace=False)
...
group_dict = get_group_dict(X_train['GroupNumber'])
X_train['GroupNumber'] = X_train['GroupNumber'].replace(group_dict, inplace=False)
group_dict = get_group_dict(X_test['GroupNumber'])
X_test['GroupNumber'] = X_test['GroupNumber'].replace(group_dict, inplace=False)
...
X_train = X_train.drop(['Cabin'], axis=1, inplace=False)
X_test = X_test.drop(['Cabin'], axis=1, inplace=False)
...
group_dict = get_group_dict(X_train['CabinNumber'])
X_train['CabinNumber'] = X_train['CabinNumber'].replace(group_dict, inplace=False)
group_dict = get_group_dict(X_test['CabinNumber'])
X_test['CabinNumber'] = X_test['CabinNumber'].replace(group_dict, inplace=False)
X_train.info()
X_train.describe()
X_train.head()
df = X_train.copy()
...
class KFoldTargetEncoder(TransformerMixin):
    ...
    def fit(self, X, y):
        ...
        for col_name in self.col_names:
            local_means[col_name] = local_means[col_name].fillna(self.global_mean, inplace=False)
    ...
    def transform(self, X):
        ...
        for col_name in self.col_names:
            ...
            X[new_col_name] = X[new_col_name].fillna(self.global_mean, inplace=False)
            X = X.drop(col_name, axis=1, inplace=False)
        return X
