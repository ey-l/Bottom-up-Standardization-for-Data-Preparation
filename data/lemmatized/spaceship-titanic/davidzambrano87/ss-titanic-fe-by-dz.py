...
def fill_nan(dataset):
    ...
    return new_dataset
train_dataset_fe = fill_nan(_input1)
import numpy as np
...
def log_transform(dataset):
    ...
    return new_dataset
train_dataset_fe = log_transform(_input1)
...
sns.heatmap(_input1.corr(), annot=True, cmap='coolwarm', ax=axes[0])
...
temp_df = _input1[['Transported', 'RoomService', 'VRDeck', 'Spa']].copy()
...
(fig, axes) = plt.subplots(1, 4, sharex=True, sharey=True, figsize=(18, 3))
temp_df = _input1[['Transported', 'FoodCourt', 'VRDeck', 'Spa']].copy()
...
sns.heatmap(temp_df.corr(), annot=True, cmap='coolwarm')
(fig, axes) = plt.subplots(1, 4, sharex=True, sharey=True, figsize=(18, 3))
temp_df = _input1[['Transported', 'FoodCourt', 'RoomService', 'VRDeck', 'Spa']].copy()
