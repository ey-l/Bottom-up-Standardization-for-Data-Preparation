...
_input1['HomePlanet']
sns.barplot(data=_input1, x='HomePlanet', y='Transported')
_input1['Destination'].unique()
sns.barplot(data=_input1, x='Destination', y='Transported')
_input1[['Deck', 'Number', 'Side']] = _input1['Cabin'].str.split('/', expand=True)
_input0[['Deck', 'Number', 'Side']] = _input0['Cabin'].str.split('/', expand=True)
bins = [0, 13, 18, 25, 200]
labels = ['<=13', '13-18', '18-25', '>25']
_input1['AgeGroup'] = pd.cut(_input1['Age'], bins=bins, labels=labels, right=False)
_input0['AgeGroup'] = pd.cut(_input0['Age'], bins=bins, labels=labels, right=False)
_input1.columns
_input1['Name'] = _input1['Name'].apply(lambda x: str(x).split(' ')[-1])
_input0['Name'] = _input0['Name'].apply(lambda x: str(x).split(' ')[-1])
_input1.isnull().sum()
_input1[['group_id', 'id_in_group']] = _input1['PassengerId'].str.split('_', expand=True)
_input0[['group_id', 'id_in_group']] = _input0['PassengerId'].str.split('_', expand=True)
_input1['total_in_group'] = _input1['group_id'].map(lambda x: pd.concat([_input1['group_id'], _input0['group_id']]).value_counts()[x])
_input0['total_in_group'] = _input0['group_id'].map(lambda x: pd.concat([_input1['group_id'], _input0['group_id']]).value_counts()[x])
Y = _input1[['PassengerId', 'Transported']]
X = _input1.drop(['Transported'], axis=1)
total_data = pd.concat([X, _input0]).reset_index()
...
total_data.loc[cond5, 'VRDeck'] = _input1.loc[cond5, 'VRDeck'].fillna(0)
...
total_data.isnull().sum()
_input1[['group_id', 'Transported', 'HomePlanet']].groupby(['HomePlanet', 'group_id']).mean().reset_index()
...
sns.histplot(x='Number', data=_input1, hue='Transported', kde=True, bins=20)
...
def object_(df):
    ...
    for col in cols:
        obj_data[col] = obj_data[col].fillna(obj_data[col].mode()[0], inplace=False)
    for i in cols:
...
test_data = pd.merge(final_data, _input0['PassengerId'], on=['PassengerId'], how='inner')
