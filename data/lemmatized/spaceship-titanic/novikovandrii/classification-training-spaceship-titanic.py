...
ax = sns.boxplot(x=_input1.Age)
plt.figure(figsize=(12, 1))
ax = sns.boxplot(x=_input1.RoomService)
plt.figure(figsize=(12, 1))
ax = sns.boxplot(x=_input1.FoodCourt)
plt.figure(figsize=(12, 1))
ax = sns.boxplot(x=_input1.ShoppingMall)
plt.figure(figsize=(12, 1))
ax = sns.boxplot(x=_input1.Spa)
plt.figure(figsize=(12, 1))
ax = sns.boxplot(x=_input1.VRDeck)
...
def handle_outliers(df: pd.DataFrame, col: str):
    ...
    return res_df
train_data = _input1.copy()
test_data = _input0.copy()
plt.figure(figsize=(12, 1))
...
def fill_missed_values(df):
    res_df = df.copy()
    res_df.VIP = res_df.VIP.fillna(False, inplace=False)
    res_df.CryoSleep = res_df.CryoSleep.fillna(False, inplace=False)
    res_df.HomePlanet = res_df.HomePlanet.fillna(res_df.HomePlanet.mode()[0], inplace=False)
    res_df.Destination = res_df.Destination.fillna(res_df.Destination.mode()[0], inplace=False)
    res_df.Age = res_df.Age.fillna(train_data.Age.median(), inplace=False)
    res_df.RoomService = res_df.RoomService.fillna(res_df.RoomService.median(), inplace=False)
    res_df.FoodCourt = res_df.FoodCourt.fillna(res_df.FoodCourt.median(), inplace=False)
    res_df.ShoppingMall = res_df.ShoppingMall.fillna(res_df.ShoppingMall.median(), inplace=False)
    res_df.Spa = res_df.Spa.fillna(res_df.Spa.median(), inplace=False)
    res_df.VRDeck = res_df.VRDeck.fillna(res_df.VRDeck.median(), inplace=False)
    return res_df
...
def create_features_from_cabin(df):
    res_df = df.copy()
    res_df.Cabin = res_df.Cabin.fillna('F/0/S', inplace=False)
