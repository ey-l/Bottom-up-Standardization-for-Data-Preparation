...
_input1 = df[:_input1.shape[0]].copy()
_input0 = df[_input1.shape[0]:].copy()
for col in df.loc[:, df.columns != 'SalePrice'].columns[:]:
    if _input0[col].isna().sum():
        print(f'TEST - {col}, {df[col].isna().sum()}')
    if _input1[col].isna().sum():
...
for i in range(4):
    for j in range(4):
        ax[i][j].scatter(x=_input1[iter_features[i][j]], y=_input1['SalePrice'], label=iter_features[i][j], c=_input1[iter_features[i][j]], cmap='viridis')
        ax[i][j].set_xlabel(iter_features[i][j])
        ax[i][j].set_ylabel('Sale Price')
plt.tight_layout()
_input1 = _input1.drop(_input1[_input1['BsmtScore'] > 60000].index, axis=0, inplace=False)
_input1 = _input1.drop(_input1[_input1['TotalBsmtSF'] > 6000].index, axis=0, inplace=False)
_input1 = _input1.drop(_input1[_input1['GrLivArea'] > 4000].index, axis=0, inplace=False)
_input1 = _input1.drop(_input1[_input1['1stFlrSF'] > 4000].index, axis=0, inplace=False)
_input1 = _input1.drop(_input1[_input1['MasVnrArea'] > 1250].index, axis=0, inplace=False)
...
X_TRAIN = df.drop('SalePrice', axis=1).to_numpy()[:_input1.shape[0]]
y_TRAIN = df['SalePrice'].to_numpy()[:_input1.shape[0]]
X_TEST = df.drop('SalePrice', axis=1).to_numpy()[_input1.shape[0]:]
