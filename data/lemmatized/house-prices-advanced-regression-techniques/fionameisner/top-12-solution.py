...
_input1.head(10)
X = pd.DataFrame(_input1.loc[:, _input1.columns != 'SalePrice'])
X = pd.concat([X, _input0], axis=0)
y = pd.DataFrame(_input1['SalePrice'])
X.info()
(fig, ax) = plt.subplots(1, 2)
ax[0].hist(_input1['SalePrice'], bins=12, edgecolor='green', facecolor='yellow')
...
X.isnull().sum().sum()
X['Functional'] = X['Functional'].fillna('Typ', inplace=False)
X['Electrical'] = X['Electrical'].fillna('SBrkr', inplace=False)
