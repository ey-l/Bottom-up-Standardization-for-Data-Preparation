...
print('How many feature candidates do we have? %d' % (len(_input1.columns) - 1))
null_in_train_csv = _input1.isnull().sum()
null_in_train_csv = null_in_train_csv[null_in_train_csv > 0]
null_in_train_csv = null_in_train_csv.sort_values(inplace=False)
null_in_train_csv.plot.bar()
sns.heatmap(_input1.corr(), vmax=0.8, square=True)
arr_train_cor = _input1.corr()['SalePrice']
...
train_meta = _input1[idx_meta].copy()
train_meta.head(n=5)
...
print('Max Fireplaces value in train.csv: %d, in test.csv: %d' % (_input1['Fireplaces'].max(), _input0['Fireplaces'].max()))
print('Min Fireplaces value in train.csv: %d, in test.csv: %d' % (_input1['Fireplaces'].min(), _input0['Fireplaces'].min()))
print('Max GarageCars value in train.csv: %d, in test.csv: %d' % (_input1['GarageCars'].max(), _input0['GarageCars'].max()))
print('Min GarageCars value in train.csv: %d, in test.csv: %d' % (_input1['GarageCars'].min(), _input0['GarageCars'].min()))
