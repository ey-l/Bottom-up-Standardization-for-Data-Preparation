...
train_frac = 0.95
_input1 = pd.read_csv('_data/input/house-prices-advanced-regression-techniques/train.csv', encoding='utf8')
_input0 = pd.read_csv('_data/input/house-prices-advanced-regression-techniques/test.csv', encoding='utf8')
_input2 = pd.read_csv('_data/input/house-prices-advanced-regression-techniques/sample_submission.csv', encoding='utf8')
print(colored(f'Train data columns are \n{_input1.columns}', color='blue'))
print(colored(f'\nTrain data details\n', color='blue', attrs=['dark', 'bold']))
_input1.info()
print(colored(f'\nData preprocessing and pre-pipeline steps\n', color='blue', attrs=['dark', 'bold']))
(xtrain, ytrain) = (_input1.drop([target, 'Id'], axis=1), _input1[[target]])
_input0 = _input0.drop('Id', axis=1, errors='ignore', inplace=False)
num_enc_feat_lst = ['OverallQual', 'OverallCond', 'MSSubClass']
xtrain[num_enc_feat_lst] = xtrain[num_enc_feat_lst].astype(str)
_input0[num_enc_feat_lst] = _input0[num_enc_feat_lst].astype(str)
...
xtrain = xtrain.drop(num_null_feat_drop_lst, axis=1, inplace=False, errors='ignore')
_input0 = _input0.drop(num_null_feat_drop_lst, axis=1, inplace=False, errors='ignore')
...
class TargetXformer(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X: pd.DataFrame, y=None, target=target, **transform_params):
        ...
        if self.xform_func_lbl == 'log':
            ...
            X1 = X1.drop(target, axis=1, inplace=False)
            X1 = X1.rename({'xform': target}, axis=1, inplace=False)
        return X1
