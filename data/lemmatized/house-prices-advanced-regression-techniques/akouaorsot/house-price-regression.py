...
sns.histplot(data=_input1, x='SalePrice')
...
def basic_processing(data, col_drop=[], col_rename={'SalePrice': 'target'}):
    ...
    """
    if len(col_drop) > 0:
        data = data.drop(col_drop, axis=1, inplace=False)
    if col_rename:
        data = data.rename(columns=col_rename, inplace=False)
    data = data.drop_duplicates(keep='first', inplace=False)
    return data
df = basic_processing(_input1)
df.head()
...
def datetime_encoding(df, cols, mapping):
    ...
    for c in cols:
        ...
        df[c] = df[c].fillna(0, inplace=False)
