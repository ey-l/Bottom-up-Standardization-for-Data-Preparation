...
_input1.head()
numerical_data = _input1.select_dtypes(include=['int64', 'float64'])
categorical_data = _input1.select_dtypes(include=['object'])
numerical_data = numerical_data.drop(['Id'], axis=1, inplace=False)
numerical_data.head()
...
numerical_data = numerical_data.drop(['SalePrice'], axis=1, inplace=False)
...
for col in date_cols:
    fig.add_trace(go.Scatter(x=_input1[col], y=_input1['SalePrice'], mode='markers', name=col))
...
for (i, col) in enumerate(categorical_data.columns[:-1]):
    _col = (i + 1) % 3
    _col = _col if _col != 0 else 3
    series = _input1.groupby(col)['SalePrice'].mean()
...
test_cat_col = list(_input0.select_dtypes(include=['object']))
_input0[test_cat_col].describe()
...
def OrdinalCols(cat_df, one_hot_cols):
    ...
    return ordinal_cols
one_hot_cols = ONEHOTCOLS(categorical_data, _input0)
