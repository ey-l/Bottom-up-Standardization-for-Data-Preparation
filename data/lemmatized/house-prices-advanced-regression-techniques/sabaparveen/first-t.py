...
g = sns.histplot(_input1.SalePrice, kde=True)
print('SalePrice Skewness is = ', _input1.SalePrice.skew())
print('Kurtosis: %f' % _input1['SalePrice'].kurt())
plt.figure(figsize=(15, 15))
sns.heatmap(_input1.corr(), cmap='autumn')
all_data = pd.concat([_input1, _input0]).reset_index(drop=True)
SalePrice = _input1['SalePrice']
all_data = all_data.drop(columns=['SalePrice'], axis=1, inplace=False)
all_data.shape
...
all_data.shape
train_data = all_data[:len(_input1)]
train_data['SalePrice'] = SalePrice
test_data = all_data[len(_input1):]
...
all_data = all_data.drop(columns=['SalePrice'], inplace=False)
all_data.shape
from scipy import stats
...
def cv_rmse(model, X=_input1, Y=target):
