...
for radek in description.splitlines():
    if radek.split(':')[0] in _input1.columns:
...
def des_df(df):
    for c in df.columns:
        if df[c].dtype == object:
            ...
            des_numeric(df, c)
des_df(_input1.select_dtypes(exclude=['object']))
df_cat_only_train = _input1.select_dtypes(include=['object']).copy()
df_cat_only_train.loc[:, 'SalePrice'] = _input1['SalePrice'].copy()
des_df(df_cat_only_train)
df_concat = pd.concat([_input1, _input0])
...
def handle_na_values(df):
    ...
    return df
_input1 = handle_na_values(_input1)
_input0 = handle_na_values(_input0)
...
for variable in ordinal_cat:
    for value in set(_input0.select_dtypes(include=['object'])[variable]):
        if not value in set(_input1.select_dtypes(include=['object'])[variable]):
            print(variable, value)
df_concat.loc[df_concat['KitchenQual'] == 'no_KitchenQual', 'KitchenQual'] = 'Fa'
for col in ordinal_cat:
    oe = OrdinalEncoder(categories=[list(_input1.groupby([col])['SalePrice'].mean().sort_values().index)])
