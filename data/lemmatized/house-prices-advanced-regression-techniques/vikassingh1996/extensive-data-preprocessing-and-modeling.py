...
print('Dimensions of train data:', _input1.shape)
print('Dimensions of test data:', _input0.shape)
"Let's check the columns names"
_input1.columns.values
"Let's merge the train and test data and inspect the data type"
merged = pd.concat([_input1, _input0], axis=0, sort=True)
...
_input1 = _input1.drop(_input1[_input1.GrLivArea > 4000].index, inplace=False)
...
scatter_plot(_input1.YearBuilt, np.log1p(_input1.SalePrice), 'YearBuilt Vs SalePrice', 'YearBuilt', 'SalePrice', 10, 'viridis')
'Drop observations where YearBulit is less than 1893 sq.ft'
_input1 = _input1.drop(_input1[_input1.YearBuilt < 1900].index, inplace=False)
_input1 = _input1.reset_index(drop=True, inplace=False)
'Scatter plot of YearBuilt Vs SalePrice'
scatter_plot(_input1.YearBuilt, np.log1p(_input1.SalePrice), 'YearBuilt Vs SalePrice', 'YearBuilt', 'SalePrice', 10, 'viridis')
'Scatter plot of GarageCars Vs SalePrice'
scatter_plot(_input1.GarageCars, np.log(_input1.SalePrice), 'GarageCars Vs SalePrice', 'GarageCars', 'SalePrice', 10, 'Electric')
'Scatter plot of GarageCars Vs SalePrice'
scatter_plot(_input1.OverallQual, np.log(_input1.SalePrice), 'OverallQual Vs SalePrice', 'OverallQual', 'SalePrice', 10, 'Bluered')
'Scatter plot of FullBath Vs SalePrice'
scatter_plot(_input1.FullBath, np.log(_input1.SalePrice), 'FullBath Vs SalePrice', 'FullBath', 'SalePrice', 10, 'RdBu')
...
df_merged = pd.concat([_input1, _input0], axis=0)
...
for i in to_impute_by_none.columns:
    df_merged[i] = df_merged[i].fillna('None', inplace=False)
...
for i in to_impute_by_mode.columns:
    df_merged[i] = df_merged[i].fillna(df_merged[i].mode()[0], inplace=False)
...
for i in to_impute_by_median.columns:
    df_merged[i] = df_merged[i].fillna(df_merged[i].median(), inplace=False)
