...
_input1.head()
unique_cols = list(_input1.columns[_input1.nunique() == 1])
print(unique_cols)
duplicated_cols = list(_input1.columns[_input1.T.duplicated()])
print(duplicated_cols)
categorical_cols = list(_input1.select_dtypes(include=['object']).columns)
print(categorical_cols)
_input1[categorical_cols].head()
...
X_test = _input0.drop(['Id', 'SalePrice'], axis=1)
y_preds = []
models = []
oof_train = np.zeros((len(X_train),))
cv = KFold(n_splits=5, shuffle=True, random_state=0)
params = {'num_leaves': 24, 'max_depth': 6, 'objective': 'regression', 'metric': 'rmse', 'learning_rate': 0.05}
