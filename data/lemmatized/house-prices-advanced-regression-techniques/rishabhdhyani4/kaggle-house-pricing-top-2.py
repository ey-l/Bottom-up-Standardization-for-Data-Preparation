...
for i in range(_input1.shape[1]):
    print(_input1.columns[i], '-', _input1.iloc[:, i].isnull().sum())
for i in range(_input0.shape[1]):
    print(_input0.columns[i], '-', _input0.iloc[:, i].isnull().sum())
fig = plt.figure(figsize=(15, 8))
sns.distplot(_input1['SalePrice'], bins=26, color='brown')
sns.set_style('white')
...
sns.heatmap(_input1.corr(), cmap=sns.diverging_palette(20, 220, n=200), mask=mask, annot=True, center=0, cbar='coolwarm')
plt.tight_layout()
...
y = _input1['SalePrice'].reset_index(drop=True)
previous_train = _input1.copy()
...
skewed_feats
fig = plt.figure(figsize=(15, 8))
sns.distplot(_input1['1stFlrSF'], bins=26, color='brown')
sns.set_style('white')
sns.set_context('poster', font_scale=2)
plt.tight_layout()
