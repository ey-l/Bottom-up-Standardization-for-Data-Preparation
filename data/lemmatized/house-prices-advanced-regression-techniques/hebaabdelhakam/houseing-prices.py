...
_input1
_input1.info()
null_rows = _input1.shape[0] - _input1.dropna(axis=0).shape[0]
null_rows
...
All_Data = All_Data.drop(Columns_Enc_OneHot, axis=1, inplace=False)
All_Data
...
All_Data = All_Data.drop(features_throw, axis=1, inplace=False)
All_Data
All_Data = All_Data.fillna(0, inplace=False)
All_Data
All_Data.info(1)
train_samples = len(_input1)
train_samples
...
Train_data_Y = _input1[['SalePrice']].reset_index().drop('Id', axis=1)
