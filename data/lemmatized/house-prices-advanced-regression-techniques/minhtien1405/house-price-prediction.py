...
for col in discret_columns:
    ...
    clean_df[col] = clean_df[col].fillna(most_frequent_value, inplace=False)
continuous_columns = ['LotFrontage', 'MasVnrArea', 'GarageYrBlt']
for col in continuous_columns:
    most_frequent_value = clean_df[col].value_counts().index[0]
    clean_df[col] = clean_df[col].fillna(clean_df[col].mean(), inplace=False)
...
def get_list_of_low_corr_columns(df):
    ...
    return list(columns)
clean_df = clean_df.drop(columns=get_list_of_low_corr_columns(clean_df), inplace=False)
clean_df.head()
...
column_informations = {}
num_values = len(_input1)
for col in _input1.columns:
    num_unique = _input1[col].nunique()
    num_nulls = round(_input1[col].isna().sum() / num_values, 2)
    d_type = _input1.dtypes[col]
    if num_unique < 30:
        info_str = '['
        value_counts = _input1[col].value_counts()
        ...
        column_informations[col] = {'d_type': d_type, 'discret': False, 'percentage_of_missing_values': num_nulls, 'single_value_weight': 0.0, 'min': _input1[col].min(), 'max': _input1[col].max(), 'mean': round(_input1[col].mean(), 2), 'median': round(_input1[col].median(), 2), 'info_str': ''}
    else:
...
clean_df = clean_df.drop(columns=['SalePrice'], inplace=False)
