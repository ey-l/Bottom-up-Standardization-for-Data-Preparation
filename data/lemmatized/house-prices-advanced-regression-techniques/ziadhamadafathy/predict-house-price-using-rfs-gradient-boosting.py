...
_input1 = _input1.drop(columns=['Id', 'Alley', 'PoolQC', 'Fence', 'MiscFeature', 'FireplaceQu'], inplace=False)
_input0 = _input0.drop(columns=['Id', 'Alley', 'PoolQC', 'Fence', 'MiscFeature', 'FireplaceQu'], inplace=False)
...
X = df.iloc[:1460, :]
_input0 = df.iloc[1460:, :]
new_train = pd.concat([X, y], axis=1)
new_train
sns.boxplot(y=new_train['SalePrice'])
sns.distplot(new_train['SalePrice'])
new_train = new_train.drop(new_train[new_train['SalePrice'] > 500000].index, inplace=False)
sns.distplot(new_train['SalePrice'])
sns.boxplot(new_train['GrLivArea'])
sns.distplot(new_train['GrLivArea'])
new_train = new_train.drop(new_train[new_train['GrLivArea'] > 3000].index, inplace=False)
sns.distplot(new_train['GrLivArea'])
new_train
