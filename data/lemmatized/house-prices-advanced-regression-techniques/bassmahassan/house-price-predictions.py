...
_input0.info()
_input1['LT_Salesprice'] = np.log1p(_input1['SalePrice'])
plt.hist(_input1['LT_Salesprice'], color='black')
_input1['LT_Salesprice'].skew()
plt.figure(figsize=(20, 20))
sns.heatmap(_input1.corr())

def data_cleaning(df):
    df['MSZoning'] = df['MSZoning'].fillna(value=df['MSZoning'].mode()[0], inplace=False)
    df = df.drop(['Alley', 'FireplaceQu', 'PoolQC', 'MiscFeature', 'Fence'], axis='columns', inplace=False)
    df['LotFrontage'] = df['LotFrontage'].fillna(df['LotFrontage'].dropna().mean(), inplace=False)
    ...
    for features in categorical_features:
        ...
        df = merged.copy()
    for feature in categorical_features:
        df = df.drop(feature, axis='columns', inplace=False)
    df = df.drop('LT_Salesprice', axis='columns', inplace=False)
    return df
