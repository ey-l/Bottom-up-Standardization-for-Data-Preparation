...
_input1.head()
_input0.head()
_input1.describe().T.style.set_properties(**{'background-color': 'grey', 'color': 'white', 'border-color': 'white'})
_input0.describe().T.style.set_properties(**{'background-color': 'grey', 'color': 'white', 'border-color': 'white'})
fig_ = _input1.hist(figsize=(25, 30), bins=50, xlabelsize=8, ylabelsize=8)
...
sns.heatmap(ax=axes[0], yticklabels=False, data=_input1.isnull(), cbar=False)
sns.heatmap(ax=axes[1], yticklabels=False, data=_input0.isnull(), cbar=False, cmap='YlGnBu')
...
for col in categoric_data:
    data[col] = data[col].fillna(data[col].value_counts().index[0], inplace=False)
x = data[numeric_data]
...
data.head()
X_train = data[:len(_input1)]
X_test = data[len(_input1):]
