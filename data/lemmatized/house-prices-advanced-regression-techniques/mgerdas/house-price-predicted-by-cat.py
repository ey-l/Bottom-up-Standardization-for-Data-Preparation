...
try:
    df = df.drop(columns=['MiscFeature', 'Fence', 'PoolQC', 'FireplaceQu', 'Alley'], inplace=False)
except KeyError:
...
df = df.drop(columns=df[dummy_features], inplace=False)
df.tail()
_input0 = df[df[target].isnull()].drop([target], axis=1).copy()
_input1 = df[df[target].notnull()].copy()
_input1.head(5)
_input1 = _input1.dropna(inplace=False)
_input0 = _input0.dropna(inplace=False)
y = _input1[target]
