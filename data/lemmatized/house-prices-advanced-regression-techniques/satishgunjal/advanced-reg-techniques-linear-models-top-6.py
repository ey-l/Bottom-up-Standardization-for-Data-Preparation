...
print(f'Shape of traning data= {_input1.shape}')
_input1.head()
_input0 = pd.read_csv('_data/input/house-prices-advanced-regression-techniques/test.csv')
print(f'Shape of test data= {_input0.shape}')
_input0.head()
_input1 = _input1.drop('Id', inplace=False, axis=1)
_input0 = _input0.drop('Id', inplace=False, axis=1)
print(f'After dropping Id feature, shape of train data: {_input1.shape}, test data: {_input0.shape}')
train_corr = _input1.corr(method='pearson')
...
sns.boxplot(data=_input1, x='OverallQual', y='SalePrice', ax=ax[0])
sns.violinplot(data=_input1, x='OverallQual', y='SalePrice', ax=ax[1])
(figure, ax) = plt.subplots(2, 1, figsize=(24, 10))
figure.tight_layout(pad=5.0)
sns.barplot(ax=ax[0], x='YearBuilt', y='SalePrice', data=_input1)
ax[0].set(xlabel='YearBuilt', ylabel='SalePrice')
ax[0].set_title('YearBuilt vs SalePrice (Correlation coefficient: 0.52)')
ax[0].set_xticklabels(ax[0].get_xticklabels(), rotation=90)
sns.barplot(ax=ax[1], x='YearRemodAdd', y='SalePrice', data=_input1)
...
sns.regplot(data=_input1, x='TotalBsmtSF', y='SalePrice', scatter_kws={'alpha': 0.2}, line_kws={'color': 'blue'}, ax=ax[0, 0])
ax[0, 0].set_title('TotalBsmtSF vs SalePrice (Correlation coefficient: 0.61)', fontsize=12)
sns.regplot(data=_input1, x='1stFlrSF', y='SalePrice', scatter_kws={'alpha': 0.2}, line_kws={'color': 'blue'}, ax=ax[0, 1])
ax[0, 1].set_title('1stFlrSF vs SalePrice (Correlation coefficient: 0.61)', fontsize=12)
sns.regplot(data=_input1, x='GrLivArea', y='SalePrice', scatter_kws={'alpha': 0.2}, line_kws={'color': 'blue'}, ax=ax[1, 0])
ax[1, 0].set_title('\nGrLivArea vs SalePrice (Correlation coefficient: 0.71)', fontsize=12)
sns.regplot(data=_input1, x='GarageArea', y='SalePrice', scatter_kws={'alpha': 0.2}, line_kws={'color': 'blue'}, ax=ax[1, 1])
ax[1, 1].set_title('\nGarageArea vs SalePrice (Correlation coefficient: 0.62)', fontsize=12)
(figure, ax) = plt.subplots(3, 2, figsize=(24, 12))
figure.tight_layout(pad=4.0)
sns.boxplot(data=_input1, x='FullBath', y='SalePrice', ax=ax[0, 0])
sns.violinplot(data=_input1, x='FullBath', y='SalePrice', ax=ax[0, 1])
...
sns.boxplot(data=_input1, x='TotRmsAbvGrd', y='SalePrice', ax=ax[1, 0])
sns.violinplot(data=_input1, x='TotRmsAbvGrd', y='SalePrice', ax=ax[1, 1])
...
sns.boxplot(data=_input1, x='GarageCars', y='SalePrice', ax=ax[2, 0])
sns.violinplot(data=_input1, x='GarageCars', y='SalePrice', ax=ax[2, 1])
...
def detect_and_remove_outliers(inline_delete=True):
    ...
    for row in range(0, nrows):
        for col in range(0, ncols):
            (min_thresold, max_thresold) = _input1[features[cnt]].quantile([min_percentile, max_percentile])
            df_outliers = _input1[(_input1[features[cnt]] < min_thresold) | (_input1[features[cnt]] > max_thresold)]
            outliers = outliers + df_outliers.index.tolist()
            ax[row][col].scatter(x=_input1[features[cnt]], y=_input1[target])
            ...
            if inline_delete:
                _input1 = _input1.drop(df_outliers.index.tolist())
    ...
    if inline_delete == False:
        print(f'Shape of train data= {_input1.shape}')
        _input1 = _input1.drop(unique_outliers)
        _input1 = _input1.reset_index(drop=True, inplace=False)
        print(f'Shape of train data= {_input1.shape}')
...
for row in range(0, nrows):
    for col in range(0, ncols):
        sns.regplot(data=_input1, x=features[cnt], y=target, scatter_kws={'alpha': 0.2}, line_kws={'color': 'blue'}, ax=ax[row, col])
...
all_data = all_data.reset_index(drop=True, inplace=False)
print(f'Shape of traning data= {all_data.shape}')
...
all_data['Alley'] = all_data['Alley'].replace(to_replace=['None', 'Grvl', 'Pave'], value=[0, 1, 2], inplace=False)
all_data['LotShape'] = all_data['LotShape'].replace(to_replace=['Reg', 'IR1', 'IR2', 'IR3'], value=[3, 2, 1, 0], inplace=False)
all_data['LandContour'] = all_data['LandContour'].replace(to_replace=['Lvl', 'Bnk', 'Low', 'HLS'], value=[3, 2, 1, 0], inplace=False)
all_data['LotConfig'] = all_data['LotConfig'].replace(to_replace=['Inside', 'FR2', 'Corner', 'CulDSac', 'FR3'], value=[0, 3, 1, 2, 4], inplace=False)
all_data['LandSlope'] = all_data['LandSlope'].replace(to_replace=['Gtl', 'Mod', 'Sev'], value=[2, 1, 0], inplace=False)
all_data['BldgType'] = all_data['BldgType'].replace(to_replace=['1Fam', '2fmCon', 'Duplex', 'TwnhsE', 'Twnhs'], value=[4, 3, 2, 1, 0], inplace=False)
all_data['RoofStyle'] = all_data['RoofStyle'].replace(to_replace=['Gable', 'Hip', 'Gambrel', 'Mansard', 'Flat', 'Shed'], value=[4, 2, 3, 1, 5, 0], inplace=False)
all_data['RoofMatl'] = all_data['RoofMatl'].replace(to_replace=['ClyTile', 'CompShg', 'Membran', 'Metal', 'Roll', 'Tar&Grv', 'WdShake', 'WdShngl'], value=[7, 6, 5, 4, 3, 2, 1, 0], inplace=False)
all_data['ExterQual'] = all_data['ExterQual'].replace(to_replace=['Ex', 'Gd', 'TA', 'Fa'], value=[3, 2, 1, 0], inplace=False)
all_data['ExterCond'] = all_data['ExterCond'].replace(to_replace=['Ex', 'Gd', 'TA', 'Fa', 'Po'], value=[4, 3, 2, 1, 0], inplace=False)
all_data['BsmtQual'] = all_data['BsmtQual'].replace(to_replace=['Ex', 'Gd', 'TA', 'Fa', 'None'], value=[4, 3, 2, 1, 0], inplace=False)
all_data['BsmtCond'] = all_data['BsmtCond'].replace(to_replace=['Gd', 'TA', 'Fa', 'Po', 'None'], value=[4, 3, 2, 1, 0], inplace=False)
all_data['BsmtExposure'] = all_data['BsmtExposure'].replace(to_replace=['Gd', 'Av', 'Mn', 'No', 'None'], value=[4, 3, 2, 1, 0], inplace=False)
all_data['BsmtFinType1'] = all_data['BsmtFinType1'].replace(to_replace=['GLQ', 'ALQ', 'BLQ', 'Rec', 'LwQ', 'Unf', 'None'], value=[6, 5, 4, 3, 2, 1, 0], inplace=False)
all_data['BsmtFinType2'] = all_data['BsmtFinType2'].replace(to_replace=['GLQ', 'ALQ', 'BLQ', 'Rec', 'LwQ', 'Unf', 'None'], value=[6, 5, 4, 3, 2, 1, 0], inplace=False)
all_data['HeatingQC'] = all_data['HeatingQC'].replace(to_replace=['Ex', 'Gd', 'TA', 'Fa', 'Po'], value=[4, 3, 2, 1, 0], inplace=False)
all_data['KitchenQual'] = all_data['KitchenQual'].replace(to_replace=['Ex', 'Gd', 'TA', 'Fa'], value=[3, 2, 1, 0], inplace=False)
all_data['Functional'] = all_data['Functional'].replace(to_replace=['Typ', 'Min1', 'Min2', 'Mod', 'Maj1', 'Maj2', 'Sev'], value=[6, 5, 4, 3, 2, 1, 0], inplace=False)
all_data['FireplaceQu'] = all_data['FireplaceQu'].replace(to_replace=['Ex', 'Gd', 'TA', 'Fa', 'Po', 'None'], value=[5, 4, 3, 2, 1, 0], inplace=False)
all_data['GarageType'] = all_data['GarageType'].replace(to_replace=['2Types', 'Attchd', 'Basment', 'BuiltIn', 'CarPort', 'Detchd', 'None'], value=[6, 5, 4, 3, 2, 1, 0], inplace=False)
all_data['GarageFinish'] = all_data['GarageFinish'].replace(to_replace=['Fin', 'RFn', 'Unf', 'None'], value=[3, 2, 1, 0], inplace=False)
all_data['GarageQual'] = all_data['GarageQual'].replace(to_replace=['Ex', 'Gd', 'TA', 'Fa', 'Po', 'None'], value=[5, 4, 3, 2, 1, 0], inplace=False)
all_data['GarageCond'] = all_data['GarageCond'].replace(to_replace=['Ex', 'Gd', 'TA', 'Fa', 'Po', 'None'], value=[5, 4, 3, 2, 1, 0], inplace=False)
all_data['PavedDrive'] = all_data['PavedDrive'].replace(to_replace=['Y', 'P', 'N'], value=[2, 1, 0], inplace=False)
all_data['Fence'] = all_data['Fence'].replace(to_replace=['GdPrv', 'MnPrv', 'GdWo', 'MnWw', 'None'], value=[4, 3, 2, 1, 0], inplace=False)
...
all_data.head()
_input1 = all_data[:len(y_train)]
_input0 = all_data[len(y_train):]
print(f'Shape of train: {_input1.shape}, test:{_input0.shape}')
