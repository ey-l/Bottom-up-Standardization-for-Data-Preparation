...
_input1.head()
corr = _input1.drop('Id', 1).corr().sort_values(by='SalePrice', ascending=False).round(2)
print(corr['SalePrice'])
sns.scatterplot(x='OverallQual', y='SalePrice', data=_input1)
sns.scatterplot(x='GrLivArea', y='SalePrice', data=_input1)
_input1 = _input1.drop(_input1.loc[(_input1['GrLivArea'] > 4000) & (_input1['SalePrice'] < 200000)].index, 0)
...
cm = np.corrcoef(_input1[cols].values.T)
cm
sns.pairplot(_input1[corr['SalePrice'].head(10).index])
trainrow = _input1.shape[0]
testrow = _input0.shape[0]
testids = _input0['Id'].copy()
...
for cols in ['PoolQC', 'MiscFeature', 'Alley', 'Fence', 'FireplaceQu', 'GarageCond', 'GarageQual', 'GarageFinish']:
    data[cols] = data[cols].fillna('NA', inplace=False)
for bsmt in ['BsmtQual', 'BsmtCond', 'BsmtExposure', 'BsmtFinType1', 'BsmtFinType2']:
    data[bsmt] = data[bsmt].fillna('NA', inplace=False)
for gar in ['GarageYrBlt', 'GarageType', 'GarageCars', 'GarageArea']:
    data[gar] = data[gar].fillna(0, inplace=False)
