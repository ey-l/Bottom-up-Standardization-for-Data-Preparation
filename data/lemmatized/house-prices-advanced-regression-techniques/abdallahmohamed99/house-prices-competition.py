...
train_num = _input1[numerical_cols].copy()
train_ord = _input1[ordinal_cols].copy()
train_nom = _input1[nominal_cols].copy()
...
train_num = train_num.drop(['LotFrontage', 'GarageYrBlt'], axis=1, inplace=False)
numerical_cols = train_num.columns

def Fill_num(df):
    meds = {}
    for col in find_nan_cols(df).index:
        df = df.fillna(df[col].median(), inplace=False)
