...
sns.scatterplot(data=_input1, x='GrLivArea', y='SalePrice', ax=ax[0][0])
sns.scatterplot(data=_input1, x='ScreenPorch', y='SalePrice', ax=ax[0][1])

def evaluate(y_predict, y_expected):
    sq_error = (np.log(y_expected) - np.log(y_predict)) ** 2
    return np.sqrt(np.mean(sq_error))
X = _input1.GrLivArea.to_numpy()
...
def gradient_descent(X, y, w_in, b_in, cost_function, gradient_function, alpha, num_iters):
    ...
    for i in range(num_iters):
        ...
        b = b - alpha * dj_db
    return (w, b)
cols = _input1.corr().SalePrice.sort_values(ascending=False)[1:11].index
_input1[cols]
y = _input1['SalePrice'].to_numpy()
X = _input1[cols].to_numpy()
X = scale_values(X)
...
X_test = _input0[cols].to_numpy()
X_test = np.nan_to_num(X_test)
X_test = scale_values(X_test)
y_test = np.sum(X_test * theta + b, axis=1)
predictions_df = pd.DataFrame({'Id': _input0['Id'].values, 'SalePrice': y_test})