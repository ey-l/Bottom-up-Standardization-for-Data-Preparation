...
_input1 = _input1.drop(_input1[_input1.SalePrice >= 700000].index)
_input1 = _input1.drop(_input1[_input1.LotFrontage >= 200].index)
_input1 = _input1.drop(_input1[_input1.LotArea >= 100000].index)
_input1 = _input1.drop(_input1[(_input1.OverallQual == 4) & (_input1.SalePrice > 200000)].index)
_input1 = _input1.drop(_input1[(_input1.OverallQual == 8) & (_input1.SalePrice > 500000)].index)
_input1 = _input1.drop(_input1[(_input1.OverallQual == 10) & (_input1.SalePrice < 300000)].index)
_input1 = _input1.drop(_input1[(_input1.OverallCond == 2) & (_input1.SalePrice > 300000)].index)
_input1 = _input1.drop(_input1[(_input1.OverallCond == 5) & (_input1.SalePrice > 700000)].index)
_input1 = _input1.drop(_input1[(_input1.OverallCond == 6) & (_input1.SalePrice > 700000)].index)
_input1 = _input1.drop(_input1[(_input1.YearBuilt <= 1900) & (_input1.SalePrice > 200000)].index)
_input1 = _input1.drop(_input1[_input1.MasVnrArea >= 1200].index)
_input1 = _input1.drop(_input1[_input1.BsmtFinSF1 >= 3000].index)
_input1 = _input1.drop(_input1[_input1.BsmtFinSF2 >= 1200].index)
_input1 = _input1.drop(_input1[_input1.TotalBsmtSF >= 4000].index)
_input1 = _input1.drop(_input1[_input1['1stFlrSF'] >= 4000].index)
_input1 = _input1.drop(_input1[(_input1.LowQualFinSF > 500) & (_input1.SalePrice > 400000)].index)
_input1 = _input1.drop(_input1[_input1.GrLivArea >= 4000].index)
_input1 = _input1.drop(_input1[_input1.BedroomAbvGr >= 7].index)
_input1 = _input1.drop(_input1[_input1.KitchenAbvGr < 1].index)
_input1 = _input1.drop(_input1[_input1.TotRmsAbvGrd > 12].index)
_input1 = _input1.drop(_input1[_input1.Fireplaces > 2].index)
_input1 = _input1.drop(_input1[_input1.GarageCars > 3].index)
_input1 = _input1.drop(_input1[(_input1.GarageArea > 1200) & (_input1.SalePrice < 300000)].index)
_input1 = _input1.drop(_input1[_input1.WoodDeckSF > 700].index)
_input1 = _input1.drop(_input1[_input1.OpenPorchSF > 450].index)
_input1 = _input1.drop(_input1[_input1.EnclosedPorch > 450].index)
_input1 = _input1.drop(_input1[_input1['3SsnPorch'] > 350].index)
full = pd.concat([_input1, _input0], ignore_index=True)
full = full.drop(['SalePrice'], axis=1, inplace=False)
words_set = set()
...
def add_columns(dataframe):
    ...
    dataframe = dataframe.drop(columns=['MiscFeature'], inplace=False)
    dataframe = dataframe.drop(columns=['MiscVal'], inplace=False)
add_columns(full)
print('Full data shape is {}'.format(full.shape))
plt.scatter(range(0, len(_input1['SalePrice'])), _input1['SalePrice'])
    ...
    try:
        for category in one_hots:
            ...
            if category not in ['Condition1', 'Condition2']:
                dataframe = dataframe.drop(columns=[category], inplace=False)
        return dataframe
    except KeyError:
...
def handleFence(dataframe):
    ...
    dataframe = dataframe.drop(columns=['Fence'], inplace=False)
    return dataframe
...
def handleUtilities(dataframe):
    ...
    dataframe = dataframe.drop(columns=['Utilities'], inplace=False)
    return dataframe
...
train_neighborhood = set(_input1['Neighborhood'].tolist())
...
geo_dataframe['SalePrice'] = pd.Series(_input1.groupby(['Neighborhood']).mean()['SalePrice'].values, index=geo_dataframe.index)
import folium
...
def add_lat_lon_columns(df):
    ...
    df = df.drop(columns=['Neighborhood'], inplace=False)
