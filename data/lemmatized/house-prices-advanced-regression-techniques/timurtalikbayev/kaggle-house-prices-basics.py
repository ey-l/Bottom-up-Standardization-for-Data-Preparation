...
important_num_cols = list(_input1.corr()['SalePrice'][(_input1.corr()['SalePrice'] > 0.3) | (_input1.corr()['SalePrice'] < -0.3)].index)
cat_cols = ['MSZoning', 'Utilities', 'BldgType', 'Heating', 'KitchenQual', 'SaleCondition', 'LandSlope']
important_cols = important_num_cols + cat_cols
train_df = _input1[important_cols]
train_df.head()
print(train_df.isna().sum())
print('Total: ', train_df.isna().sum().sum())
sns.scatterplot(x='SalePrice', y='GarageYrBlt', data=train_df)
_input1['GarageYrBlt'].describe()
...
train_df['LotFrontage'] = _input1.groupby('Neighborhood')['LotFrontage'].transform(lambda x: x.fillna(x.median()))
...
def get_dataframe_with_dummy_vars(df, list_of_columns):
    for column in list_of_columns:
        ...
        dummy_df = dummy_df.reset_index(inplace=False)
        for (index, value) in enumerate(dummy_df['index']):
            name = dummy_df.iloc[index, 0]
            new_column_name = column + '_' + name
            dummy_var_df = dummy_var_df.rename(columns={name: new_column_name}, inplace=False)
