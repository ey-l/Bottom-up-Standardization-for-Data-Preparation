...
_input1 = _input1.drop(cols_to_drop, axis=1)
...
for col in num_X:
    num_X[col] = num_X[col].fillna(num_X[col].mean(), inplace=False)
for col in num_X:
...
X_test = _input0.drop(columns=cols_to_drop, axis=1)
...
for col in num_X_test:
    num_X_test[col] = num_X_test[col].fillna(num_X_test[col].mean(), inplace=False)
