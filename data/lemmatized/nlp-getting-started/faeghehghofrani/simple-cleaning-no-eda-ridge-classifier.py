...
def Info_dataFrame(df):
    ...
    print(df.isnull().sum())
    print('==' * 20)
Info_dataFrame(_input1)
Info_dataFrame(_input0)
...
def clean_text_Simple(df):
    for i in range(len(df)):
        ...
        df['text'][i] = corpus
    return df
train_df_m1 = clean_text_Simple(_input1)
train_df_m1
test_df_m1 = clean_text_Simple(_input0)
test_df_m1
...
def clean_text_Porter(df):
    for i in range(len(df)):
        ...
        df['text'][i] = corpus
    return df
train_df_mp = clean_text_Porter(_input1)
train_df_mp
test_df_mp = clean_text_Porter(_input0)
test_df_mp
