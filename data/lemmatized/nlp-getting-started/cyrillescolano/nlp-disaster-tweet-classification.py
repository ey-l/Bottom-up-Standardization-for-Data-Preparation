...
print(_input1.shape, _input0.shape, _input2.shape)
_input1.info()
_input1.head()
print(_input1.duplicated().sum())
print(_input0.duplicated().sum())
...
sns.barplot(x=_input1[missing_cols].isnull().sum().index, y=_input1[missing_cols].isnull().sum().values, ax=axes[0])
sns.barplot(x=_input0[missing_cols].isnull().sum().index, y=_input0[missing_cols].isnull().sum().values, ax=axes[1])
...
print('Target of 0 is {} % of total'.format(round(_input1['target'].value_counts()[0] / len(_input1['target']) * 100)))
print('Target of 1 is {} % of total'.format(round(_input1['target'].value_counts()[1] / len(_input1['target']) * 100)))
feat = _input1['keyword'].value_counts()
print(feat.head())
fig = px.scatter(feat, x=feat.values, y=feat.index, size=feat.values)
fig.show()
feat = _input1['location'].value_counts()
print(feat.head())
fig = px.scatter(feat, x=feat.values, y=feat.index, size=feat.values)
fig.show()
...
_input1['location'] = _input1['location'].replace(loc_dict, inplace=False)
sns.barplot(y=_input1['location'].value_counts()[:10].index, x=_input1['location'].value_counts()[:10], orient='h')
_input1 = _input1.drop(['location'], axis=1)
_input1.head()
_input1['tweet_len'] = _input1['text'].apply(lambda x: len(x))
_input1.head(4)
_input1.tweet_len.describe()
ax = sns.distplot(_input1['tweet_len']).set_title('Distribution of the tweet lengths')
plt.grid(True)
(f, (ax1, ax2)) = plt.subplots(1, 2, sharex=True, figsize=(10, 6))
sns.distplot(_input1[_input1['target'] == 1]['tweet_len'], ax=ax1, kde=False, color='green', label='Disater texts')
sns.distplot(_input1[_input1['target'] == 0]['tweet_len'], ax=ax2, kde=False, color='red', label='Non-Disater texts')
f.suptitle('tweet length distribution')
f.legend(loc='upper right')
ax1.grid()
ax2.grid()
(fig, (ax1, ax2)) = plt.subplots(1, 2, figsize=(10, 5))
dis_text = _input1[_input1['target'] == 1]['text'].str.split().map(lambda x: len(x))
ax1.hist(dis_text, color='blue')
ax1.set_title('Disaster tweets')
ax1.grid()
nondis_text = _input1[_input1['target'] == 0]['text'].str.split().map(lambda x: len(x))
ax2.hist(nondis_text, color='red')
ax2.set_title('Non-disaster tweets')
ax2.grid()
fig.suptitle('Words in a text')
_input1[_input1['target'] == 1]['text'][10:20]
_input1[_input1['target'] == 0]['text'][10:20]
_input1['text'][:10]
...
def clean_text(text):
    ...
    return text
train_clean = _input1.copy()
test_clean = _input0.copy()
...
text_after_preprocess(_input1.text, train_clean.text)
text_after_preprocess(_input0.text, test_clean.text)
train_clean.head()
train_clean.head()
