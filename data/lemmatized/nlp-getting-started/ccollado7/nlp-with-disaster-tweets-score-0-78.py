...
_input1.location = _input1.location.fillna('None', inplace=False)
_input1.keyword = _input1.keyword.fillna('None', inplace=False)
_input1.head()
duplicados_train = _input1['text'].duplicated(keep=False)
duplicados_train.sum()
dup_train = _input1[['text', 'target']][duplicados_train]
dup_train
_input1.shape
_input1 = _input1.drop_duplicates('text', keep=False)
_input1.head()
_input1.shape
_input0.head()
_input0.shape
_input0.location = _input0.location.fillna('None', inplace=False)
_input0.keyword = _input0.keyword.fillna('None', inplace=False)
_input0.head()
duplicados_test = _input0['text'].duplicated(keep=False)
duplicados_test.sum()
dup_test = _input0[['text']][duplicados_test]
dup_test
...
preprocesar_df(_input1, 'text', 'texto_preprocesado')
_input1.head(30)
_input1.shape
x_train_original = _input1['text']
x_train_preprocesado = _input1['texto_preprocesado']
y_train = _input1['target']
preprocesar_df(_input0, 'text', 'texto_preprocesado')
_input0.head()
_input0.shape
x_test_original = _input0['text']
x_test_preprocesado = _input0['texto_preprocesado']
