...
def tokenize_df(dfs: List[pd.DataFrame]) -> List[str]:
    ...
    return tokens
tokens_all = tokenize_df([_input1, _input0])
tokens_train = tokenize_df(_input1)
tokens_test = tokenize_df(_input0)
tokens_disaster = tokenize_df(_input1[_input1['target'] == 1])
tokens_not_disaster = tokenize_df(_input1[_input1['target'] == 0])
...
def tf_idf_classifer_accuracy(df: pd.DataFrame, tf_idf_ratio_disaster, tf_idf_ratio_not_disaster) -> float:
    ...
    return accuracy
accuracy = tf_idf_classifer_accuracy(_input1, tf_idf_ratio_disaster, tf_idf_ratio_not_disaster)
print('accuracy =', accuracy)
_input1
predictions = tf_idf_classifer_df(_input1)
tokens_true_positive = tokenize_df(_input1[_input1['target'] == predictions])
tokens_false_positive = tokenize_df(_input1[_input1['target'] != predictions])
...
def ratio_hyperparameter_search():
    ...
    for scale1 in [0.5, 1, 1.5, 2, 2.5, 3, 4, 8, 16]:
        for scale2 in [0.5, 1, 1.5, 2, 2.5, 3, 4, 8, 16]:
            ...
            accuracy = tf_idf_classifer_accuracy(_input1, tf_idf_ratio_disaster + tf_idf_true_positive_ratio_scaled, tf_idf_ratio_not_disaster + tf_idf_false_positive_ratio_scaled)
...
accuracy = tf_idf_classifer_accuracy(_input1, tf_idf_ratio_disaster + tf_idf_true_positive_ratio_scaled, tf_idf_ratio_not_disaster + tf_idf_false_positive_ratio_scaled)
print('accuracy =', accuracy)
_input2 = pd.read_csv('data/input/nlp-getting-started/sample_submission.csv', index_col=0)
for (index, row) in _input0.iterrows():
