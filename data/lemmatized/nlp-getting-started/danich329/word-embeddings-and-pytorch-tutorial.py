...
_input1.head()
import gensim
_input1.text.apply(lambda x: x.split(' ')).head().values
training_corpus = _input1.text.apply(lambda x: x.split(' '))
...
sentence_length = _input1.text.apply(lambda x: x.split(' ')).apply(len).max()
sentence_length
word_map = {word: idx for (idx, word) in enumerate(model.wv.vocab, start=2)}
word_map
_input1.text.apply(lambda x: x.split(' '))[0]
training_sentence_data = _input1.text.apply(lambda x: list(map(lambda word: word_map[word] if word in word_map else 1, x.split(' '))))
...
dataset = data.TensorDataset(training_sentence_data, torch.FloatTensor(_input1.target.values))
