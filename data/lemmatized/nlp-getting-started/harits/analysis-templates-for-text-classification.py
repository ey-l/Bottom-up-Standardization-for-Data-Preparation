...
_input1
print()
_input1['char_count'] = _input1['text'].apply(lambda x: len(x))
_input1['list_word'] = _input1['text'].str.split()
_input1['word_count'] = _input1['list_word'].apply(lambda x: len(x))
_input1['unique_word_count'] = _input1['list_word'].apply(lambda x: len(set(x)))
_input1['mean_word_length'] = _input1['list_word'].apply(lambda x: [len(i) for i in x]).apply(lambda x: np.mean(x))

def find_specific_words(pattern, string):
    words = re.findall(pattern, string)
    return [x[0] for x in words]
stop = set(stopwords.words('english'))
_input1['stopword_count'] = _input1['list_word'].apply(lambda x: len([word for word in x if word in stop]))
_input1['unstopword_count'] = _input1['list_word'].apply(lambda x: len([word for word in x if word not in stop]))
punctuations = string.punctuation
_input1['punc_count'] = _input1['text'].apply(lambda x: len([c for c in x if c in punctuations]))
url_pattern = '(?i)\\b((?:https?://|www\\d{0,3}[.]|[a-z0-9.\\-]+[.][a-z]{2,4}/)(?:[^\\s()<>]+|\\(([^\\s()<>]+|(\\([^\\s()<>]+\\)))*\\))+(?:\\(([^\\s()<>]+|(\\([^\\s()<>]+\\)))*\\)|[^\\s`!()\\[\\]{};:\'\\".,<>?«»“”‘’]))'
_input1['url_count'] = _input1['text'].apply(lambda x: len(find_specific_words(url_pattern, x)))
hashtag_pattern = '#([A-Za-z0-9]+)'
_input1['hashtag_count'] = _input1['text'].apply(lambda x: len(find_specific_words(hashtag_pattern, x)))
mention_pattern = '@([A-Za-z0-9]+)'
_input1['mention_count'] = _input1['text'].apply(lambda x: len(find_specific_words(mention_pattern, x)))
_input1['upper_case_count'] = _input1['text'].apply(lambda x: len([c for c in x if c.isupper()]))
_input1['lower_case_count'] = _input1['text'].apply(lambda x: len([c for c in x if c.islower()]))
print(_input1.shape)

def draw_diverge_colors_histogram(x_label, bins, title, log=False):
    (fig, ax) = plt.subplots(figsize=(15, 5.5))
    df_count = _input1[[x_label, 'target']]
...
def get_top_count_vectorizer(sentence_list, ngram, n):
    ...
    df_vectorizer['target'] = _input1['target']
...
(target_0_top_count, target_1_top_count) = get_top_count_vectorizer(_input1['text'], ngram=1, n=10)
draw_two_hor_bar([target_0_top_count, target_1_top_count], 'Unigram')
(target_0_top_count, target_1_top_count) = get_top_count_vectorizer(_input1['text'], ngram=2, n=10)
draw_two_hor_bar([target_0_top_count, target_1_top_count], 'Bigram')
(target_0_top_count, target_1_top_count) = get_top_count_vectorizer(_input1['text'], ngram=3, n=10)
