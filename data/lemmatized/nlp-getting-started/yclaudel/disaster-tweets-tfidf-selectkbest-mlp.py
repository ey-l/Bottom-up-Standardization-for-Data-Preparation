...
_input1['keyword'] = _input1['keyword'].map(lambda x: x.replace('%20', ' '))
_input0['keyword'] = _input0['keyword'].fillna('', inplace=False)
_input0['keyword'] = _input0['keyword'].map(lambda x: x.replace('%20', ' '))
...
stop_word.append('û_')
_input1['target'].value_counts().plot(kind='barh')
...
plot_sample_length_distribution(_input1['text'])
keyword_stats = _input1.groupby('keyword').agg({'text': np.size, 'target': np.mean}).rename(columns={'text': 'Count', 'target': 'Disaster Probability'})
...
STOPWORDS.add('CO')
STOPWORDS.add('û_')
no_disaster_text = ' '.join(_input1[_input1['target'] == 0].text.to_numpy().tolist())
real_disaster_text = ' '.join(_input1[_input1['target'] == 1].text.to_numpy().tolist())
...
X_train = vect.fit_transform(_input1['text'])
