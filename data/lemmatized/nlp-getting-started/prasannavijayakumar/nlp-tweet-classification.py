...
_input1['text_clean'] = _input1['text'].apply(lambda x: remove_html(x))
_input1['text_clean'] = _input1['text'].apply(lambda x: remove_urls(x))
_input1['text_clean'] = _input1['text'].apply(lambda x: remove_emoticons(x))
_input1['text_clean'] = _input1['text'].apply(lambda x: remove_emoji(x))
_input1['text_clean'] = _input1['text'].apply(lambda x: remove_punctuation(x))
...
def process_text(df):
    ...
    return df
_input1 = process_text(_input1)
tweet_words = ''
for val in _input1.text_clean:
    val = str(val)
    tokens = val.split()
    for i in range(len(tokens)):
        tokens[i] = tokens[i].lower()
...
plt.axis('off')
disastertweet_words = ''
for val in _input1[_input1['target'] == 1].text_clean:
    val = str(val)
    tokens = val.split()
    for i in range(len(tokens)):
        tokens[i] = tokens[i].lower()
...
for val in _input1[_input1['target'] == 0].text_clean:
    val = str(val)
    tokens = val.split()
    for i in range(len(tokens)):
        tokens[i] = tokens[i].lower()
...
tfidf_text_clean = tfidf_vectorizer_text_clean.fit_transform(_input1.text_clean)
count_hashtags = count_vectorizer_hashtags.fit_transform(_input1.hashtags)
...
_input1 = _input1.join(train_text_clean, rsuffix='_count_text_clean')
_input1 = _input1.join(train_hashtags, rsuffix='_count_hashtags')
print(_input1.shape)
features_to_drop = ['id', 'keyword', 'location', 'text', 'text_clean', 'hashtags', 'target_count_text_clean']
final_df = _input1.drop(columns=features_to_drop, axis=1)
final_df.shape
y = final_df['target']
