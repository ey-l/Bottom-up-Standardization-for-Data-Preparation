...
_input1.shape
_input1.info()
_input1.isna().sum() / _input1.shape[0] * 100
_input1 = _input1.drop(columns=['id', 'location'], inplace=False)
_input1 = _input1.dropna(inplace=False)
_input1.shape
_input1.duplicated().sum()
_input1 = _input1.drop_duplicates(keep='first', inplace=False)
_input1['content'] = _input1['keyword'] + ' ' + _input1['text']
_input1.head()
_input1.iloc[2, 3]
import nltk
import string
...
_input1['transformed_content'] = _input1['content'].apply(text_preprocessing)
final_df = _input1.drop(['text', 'keyword', 'content'], axis=1)
