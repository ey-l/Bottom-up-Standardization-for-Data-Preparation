...
def cleaningfun(df):
    ...
    for sent in list(df['text']):
        ...
        corpus.append(sent)
    return corpus
corpus_train = cleaningfun(_input1)
corpus_test = cleaningfun(_input0)
hidden_train = pd.DataFrame(corpus_train, index=_input1.id, columns=['text'])
hidden_test = pd.DataFrame(corpus_test, index=_input0.id, columns=['text'])
hidden_train
...
Mat_test.shape
X = pd.DataFrame(data=Mat_train.toarray(), index=_input1.id, columns=tfidf_tokens_train)
X
test_data = pd.DataFrame(data=Mat_test.toarray(), index=_input0.id, columns=tfidf_tokens_train)
test_data.head()
y = _input1['target']
