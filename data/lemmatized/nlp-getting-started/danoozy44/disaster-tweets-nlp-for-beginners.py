...
word_tokens = word_tokenize(_input1['text'].iloc[2])
print(word_tokens)
...
remove_stopwords(_input1)
remove_stopwords(_input0)
count_vectorizer = text.CountVectorizer()
train_vectors = count_vectorizer.fit_transform(_input1['text'])
test_vectors = count_vectorizer.transform(_input0['text'])
...
svc = LinearSVC()
mnb = MultinomialNB()
clf_scores = cross_val_score(clf, train_vectors, _input1['target'], cv=10, scoring='f1')
logreg_scores = cross_val_score(logreg, train_vectors, _input1['target'], cv=10, scoring='f1')
sgd_scores = cross_val_score(sgd, train_vectors, _input1['target'], cv=10, scoring='f1')
svc_scores = cross_val_score(svc, train_vectors, _input1['target'], cv=10, scoring='f1')
mnb_scores = cross_val_score(mnb, train_vectors, _input1['target'], cv=10, scoring='f1')
