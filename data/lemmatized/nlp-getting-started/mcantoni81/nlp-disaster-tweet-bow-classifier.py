...
def pp_remove_na(data):
    data['keyword'] = data['keyword'].fillna('unknown', inplace=False)
    data['location'] = data['location'].fillna('unknown', inplace=False)
    return data
_input1 = pp_remove_na(_input1)
_input1[_input1.location != 'unknown']
print('Location cardinality: ', len(_input1['location'].unique()))
print('Keywords cardinality: ', len(_input1['keyword'].unique()))
...
def filter_cat(t_hold, serie):
    ...
    print('None filtered')
    return cat_list
location_cat = filter_cat(0.5, _input1.location)
keyword_cat = filter_cat(0.5, _input1.keyword)
...
def pp_transform_categories(data, lc, kc):
    ...
    return data
_input1 = pp_transform_categories(_input1, location_cat, keyword_cat)
print(len(_input1['keyword'].unique()))
print(len(_input1['location'].unique()))
...
def pp_lemmatize_text(data, nlp, sw):
    ...
    return Xdf
Xdf = pp_lemmatize_text(_input1, nlp, my_stopwords)
Xdf
CV = CountVectorizer()
