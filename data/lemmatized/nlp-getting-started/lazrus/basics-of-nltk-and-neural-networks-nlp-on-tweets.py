...
print('Number of Missing Values in Target feature: {}'.format(_input1.target.isnull().sum()))
(canv, axs) = plt.subplots(1, 2, figsize=(22, 8))
color = ['darkgreen', 'darkslategrey']
plt.sca(axs[0])
plt.pie(_input1.groupby('target').count()['id'], explode=(0.1, 0), startangle=120, colors=color, textprops={'fontsize': 15}, labels=['Not Disaster (57%)', 'Disaster (43%)'])
plt.sca(axs[1])
bars = plt.bar([0, 0.5], _input1.groupby('target').count()['id'], width=0.3, color=color)
...
for (bar, val) in zip(bars, _input1.groupby('target').count()['id']):
...
train_na = _input1.isnull().sum() / len(_input1) * 100
train_na = train_na.drop(train_na[train_na == 0].index).sort_values(ascending=False)
pd.DataFrame({'Train Missing Ratio': train_na}).head(3)
test_na = _input0.isnull().sum() / len(_input0) * 100
