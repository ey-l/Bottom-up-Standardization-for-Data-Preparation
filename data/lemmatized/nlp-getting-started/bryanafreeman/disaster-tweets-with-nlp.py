...
_input1 = pd.read_csv('data/input/nlp-getting-started/train.csv', encoding='utf8')
print('Train data loaded.')
clean_copy = _input1.copy()
_input0 = pd.read_csv('data/input/nlp-getting-started/test.csv', encoding='utf8')
print('Test data loaded.')
_input2 = pd.read_csv('data/input/nlp-getting-started/sample_submission.csv', encoding='utf8')
print('There are {} of records in the train data set.'.format(len(_input1.index)))
print('There are {} of records in the test data set.'.format(len(_input0.index)))
_input1.head()
_input1.info()
_input1.isnull().sum()
target_value_counts = _input1.target.value_counts()
...
train_empties = _input1[cols_with_missing].isnull().sum() / len(_input1) * 100
(fig, ax) = plt.subplots()
fig.set_size_inches(11, 8)
clrs = ['#3498db', '#e74c3c']
...
keyword_value_counts = _input1['keyword'].value_counts()
...
true_ratios = _input1.groupby('keyword')['target'].mean().sort_values(ascending=False)
(fig, ax) = plt.subplots()
fig.set_size_inches(16, 8)
...
loc_value_counts = _input1['location'].value_counts()
...
wc(_input1[_input1['target'] == 1]['text'], stop_words, max_words, 'black', 'Most Frequent Words - Real')
wc(_input1[_input1['target'] == 0]['text'], stop_words, max_words, 'black', 'Most Frequent Words - Fake')
(fig, (ax1, ax2)) = plt.subplots(1, 2, figsize=(16, 8))
tweet_len_real = _input1[_input1['target'] == 1]['text'].str.len()
sns.distplot(tweet_len_real, ax=ax1, color='#e74c3c')
ax1.set_title('Real Disaster')
tweet_len_fake = _input1[_input1['target'] == 0]['text'].str.len()
sns.distplot(tweet_len_fake, ax=ax2, color='#2ecc71')
ax2.set_title('Fake Disaster')
fig.suptitle('Length in Characters')
(fig, (ax1, ax2)) = plt.subplots(1, 2, figsize=(16, 8))
words_real = _input1[_input1['target'] == 1]['text'].str.split().apply(lambda x: [len(i) for i in x])
words_fake = _input1[_input1['target'] == 0]['text'].str.split().apply(lambda x: [len(i) for i in x])
...
def text_feature_eng(x):
    ...
    x = x.drop('text', axis=1, inplace=False)
    x.avg_word_len.fillna(0)
    return x
_input1 = clean_copy.copy()
X = _input1.copy()
