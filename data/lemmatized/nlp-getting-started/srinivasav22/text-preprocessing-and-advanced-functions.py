...
def count_values_in_column(data, feature):
    ...
    return pd.concat([total, percentage], axis=1, keys=['Total', 'Percentage'])
count_values_in_column(_input1, 'target')

def unique_values_in_column(data, feature):
    unique_val = pd.Series(data.loc[:, feature].unique())
    return pd.concat([unique_val], axis=1, keys=['Unique Values'])
unique_values_in_column(_input1, 'target')
...
duplicated_values_data(_input1)
_input1.describe()
...
find_url(sentence)
_input1['url'] = _input1['text'].apply(lambda x: find_url(x))
...
find_emoji(sentence)
_input1['emoji'] = _input1['text'].apply(lambda x: find_emoji(x))
...
find_email(sentence)
_input1['email'] = _input1['text'].apply(lambda x: find_email(x))
...
find_hash(sentence)
_input1['hash'] = _input1['text'].apply(lambda x: find_hash(x))
...
find_at(sentence)
_input1['at_mention'] = _input1['text'].apply(lambda x: find_at(x))
...
_input1['number'] = _input1['text'].apply(lambda x: find_number(x))

def find_phone_number(text):
    line = re.findall('\\b\\d{10}\\b', text)
    return ''.join(line)
find_phone_number('9998887776 is a phone number of Mark from 210,North Avenue')
_input1['phone_number'] = _input1['text'].apply(lambda x: find_phone_number(x))
...
find_year(sentence)
_input1['year'] = _input1['text'].apply(lambda x: find_year(x))
...
_input1['non_alp'] = _input1['text'].apply(lambda x: find_nonalp(x))
...
_input1['punctuation'] = _input1['text'].apply(lambda x: find_punct(x))
...
_input1['stop_words'] = _input1['text'].apply(lambda x: stop_word_fn(x))
