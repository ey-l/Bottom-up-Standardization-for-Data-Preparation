...
_input0 = pd.read_csv('_data/input/competitive-data-science-predict-future-sales/sales_train.csv', parse_dates=[0])
from datetime import datetime
f1 = '%d.%m.%Y'
my_parser = lambda date: pd.datetime.strptime(date, f1)
_input0 = pd.read_csv('_data/input/competitive-data-science-predict-future-sales/sales_train.csv', index_col=[0], parse_dates=[0], date_parser=my_parser)
_input0 = _input0.reset_index()
...
df1m3 = pd.merge(_input0, _input4, on='item_id', how='left')
df1m32 = pd.merge(df1m3, _input3, on='item_category_id', how='left')
df1m324 = pd.merge(df1m32, _input1, on='shop_id', how='left')
