...
_input0 = pd.read_csv('_data/input/competitive-data-science-predict-future-sales/sales_train.csv', parse_dates=['date'], infer_datetime_format=True, dayfirst=True)
...
df = _input0.groupby([_input0.date.apply(lambda x: x.strftime('%Y-%m')), 'item_id', 'shop_id']).sum().reset_index()
df = df[['date', 'item_id', 'shop_id', 'item_cnt_day']]
df['item_cnt_day'] = df['item_cnt_day'].clip(0.0, 20.0, inplace=False)
df = df.pivot_table(index=['item_id', 'shop_id'], columns='date', values='item_cnt_day', fill_value=0).reset_index()
df = pd.merge(_input2, df, on=['item_id', 'shop_id'], how='left').fillna(0)
main = pd.DataFrame()
