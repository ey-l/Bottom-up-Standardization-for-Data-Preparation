...
ax = (train_df.loc[:, mask[mask == True].keys()].isna().mean().sort_values() * 100).plot(kind='barh', figsize=(30, 12), xlabel='Variables', title='% of missing values in Train DF')
for container in ax.containers:
    ax.bar_label(container, fmt='%.2f%%', padding=5)
mask = test_df.isna().mean().sort_values() > 0
ax = (test_df.loc[:, mask[mask == True].keys()].isna().mean().sort_values() * 100).plot(kind='barh', figsize=(30, 15), xlabel='Variables', title='% of missing values in Test DF', grid=False)
for container in ax.containers:
    ax.bar_label(container, fmt='%.2f%%', padding=5)
