...
print(test_df.shape)
train_df.columns.drop('SalePrice') == test_df.columns
train_df['SalePrice'].isnull().sum()
all_df = pd.concat([train_df.drop('SalePrice', axis=1), test_df], axis=0, sort=False)
all_df.head()
len(all_df)
...
for (i, row) in nan_table.iterrows():
    if nan_table.loc[i].data_type == 'float64':
        all_df[i].fillna(nan_table.loc[i].favourable_value, inplace=True)
    elif search(arr_na, i):
        all_df[i].fillna('NA', inplace=True)
...
all_df.columns
train_df_no_nan = all_df[:len(train_df)]
train_df_no_nan['SalePrice'] = train_df['SalePrice']
...
test_df_no_nan = all_df[len(train_df):]
...
x_train = train_test_clean_df[:len(train_df)]
x_test = train_test_clean_df[len(train_df):]
