...
df.describe()
df.skew()
df.skew().index[df.skew().values > 1]
outlier_cols = df.skew().index[df.skew().values > 1]
df.isnull().sum().index[df.isnull().sum().values > 0]
df.dtypes.unique()
df_copy = df.copy()
df_copy.shape
...
for cols in cat_columns:
    df_copy[cols].fillna(df_copy[cols].mode()[0], inplace=True)
...
for cols in num_columns:
    if cols in outlier_cols:
        ...
        df_copy[cols].fillna(df_copy[cols].mean(), inplace=True)
