...
def print_unique_values_df(df: pd.DataFrame):
    for col in list(df):
        ...
        print('-' * 150)
print_unique_values_df(df_train)
...
def show_percentage_missing(df: pd.DataFrame, sort_val: bool=True, ascending: bool=False, color: str='firebrick', col_name='percent_missing', title: str='Percentage of data missing.', opacity: float=1.0):
    ...
    fig.show()
print(f'Are the columns in the train and test the same?: {set(list(df_train.columns)) == set(list(df_test.columns))}')
...
show_percentage_missing(df=df_train, ascending=False, sort_val=True, title='Percentage of data missing (Train)', color='purple')
show_percentage_missing(df=df_test, ascending=False, sort_val=True, title='Percentage of data missing (Test)', color='goldenrod')
...
fig.update_layout(title_text='Lot Frontage Distributions')
fig.show()
df_train_categorical = df_train.select_dtypes(exclude=['float', 'int'])
