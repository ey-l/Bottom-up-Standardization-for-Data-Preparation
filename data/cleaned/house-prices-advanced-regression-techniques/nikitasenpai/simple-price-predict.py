...
corr = Train_df.corr()
ax = sns.heatmap(corr)
train_features = [col for col in Train_df.columns if Train_df[col].isna().sum() == 0]
test_features = [col for col in Test_df.columns if Test_df[col].isna().sum() == 0]
train_df = Train_df[train_features].copy()
test_df = Test_df[test_features].copy()
