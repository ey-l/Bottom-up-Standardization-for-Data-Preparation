...
train_ind1 = house_train.index
test_ind1 = house_test.index
pd.set_option('display.max_rows', None)
pd.set_option('display.max_columns', None)
house_all = pd.concat([house_train, house_test], axis=0, sort=False)
...
sns.boxplot(x=house_train['SalePrice'], ax=ax[1])
...
house_all.reset_index(drop=True, inplace=True)
...
missing_vals
house_all.drop(['PoolQC', 'PoolArea', 'MiscFeature', 'MiscVal', 'Alley', 'Fence'], axis=1, inplace=True)
house_all[house_all.loc[:, ['GarageCond', 'GarageFinish', 'GarageQual', 'GarageYrBlt', 'GarageType']].isnull().all(axis=1)].shape
house_all['GarageYrBlt'].fillna(house_all['YearBuilt'], inplace=True)
house_all['GarageYrBlt'] = house_all['GarageYrBlt'].astype(int)
house_all['GarageCond'].fillna('No', inplace=True)
house_all['GarageQual'].fillna('No', inplace=True)
house_all['GarageFinish'].fillna('No', inplace=True)
house_all['GarageType'].fillna('No', inplace=True)
house_all['FireplaceQu'].fillna('No', inplace=True)
bsmt_fill_inds = house_all[house_all.loc[:, ['BsmtQual', 'BsmtCond', 'BsmtExposure', 'BsmtFinType1', 'BsmtFinType2']].isnull().all(axis=1)].index
house_all['BsmtQual'].fillna('No', inplace=True)
house_all['BsmtCond'].fillna('No', inplace=True)
house_all['BsmtExposure'].fillna('No', inplace=True)
house_all['BsmtFinType1'].fillna('No', inplace=True)
house_all['BsmtFinType2'].fillna('No', inplace=True)
house_all['GarageCars'].fillna(0, inplace=True)
house_all['GarageArea'].fillna(0, inplace=True)
