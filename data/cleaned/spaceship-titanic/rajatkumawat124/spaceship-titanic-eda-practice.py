...
train.shape
train['Floor'] = train['Cabin'].str[:1]
train['CabinPort'] = train['Cabin'].str[-1:]
all_cols = train.columns
for col in all_cols:
    print(f'{col} has {train[col].isna().sum()} null values and {train[col].nunique()} unique values')
...
for col in num_cols:
    print(f'{col} has {train[col].isna().sum()} null values')
(fig, axes) = plt.subplots(2, 3, figsize=(25, 10))
for (i, col) in zip(range(6), num_cols):
