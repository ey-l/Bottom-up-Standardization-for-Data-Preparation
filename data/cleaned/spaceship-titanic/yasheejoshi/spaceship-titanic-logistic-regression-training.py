...
train_data
train_data['cabin_side'] = train_data.apply(lambda x: str(x['Cabin'])[-1:], axis=1)
train_data
train_data.info()
train_data.describe()
fig = px.histogram(train_data, x='HomePlanet', color='Destination')
fig.show()
fig = px.histogram(train_data, x='CryoSleep', color='Transported')
fig.show()
train_data_age = train_data[(train_data['Age'] > 20) & (train_data['Age'] < 40)]
...
input_cols = train_data.columns[1:-2].tolist() + ['cabin_side']
