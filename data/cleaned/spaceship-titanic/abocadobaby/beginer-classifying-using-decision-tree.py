...
sns.kdeplot(data=train, x='Age', hue='Transported', palette=['black', 'purple'], multiple='stack')
...
for (i, var) in enumerate(obj_cols):
    plt.subplot(6, 1, i + 1)
    sns.countplot(data=train, x=var, hue='Transported', palette=['black', 'purple'])
...
for (i, var) in enumerate(numeric_cols):
    plt.subplot(4, 2, i + 1)
    sns.histplot(data=train, x=var, hue='Transported', palette=['black', 'purple'], kde=True, multiple='dodge', bins=50)
