...
for feat in sorted(train_data.columns):
    if train_data[feat].dtype not in [np.object, np.str] or len(train_data[feat].unique()) < 100:
        plt.hist(train_data[train_data['Transported'] == 0][feat], alpha=0.5)
        plt.hist(train_data[train_data['Transported'] == 1][feat], alpha=0.5)
        plt.title(feat)
        plt.legend(['Not transported', 'Transported'])

plt.hist(train_data[train_data['Transported'] == 0]['pid1'].map(lambda x: int(x / 300)), alpha=0.5, bins=31)
plt.hist(train_data[train_data['Transported'] == 1]['pid1'].map(lambda x: int(x / 300)), alpha=0.5, bins=31)
train_data.columns
