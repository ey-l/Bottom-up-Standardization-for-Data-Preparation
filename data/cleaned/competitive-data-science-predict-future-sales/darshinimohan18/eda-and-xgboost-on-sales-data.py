...
df.head()
df.columns
params = {'learning_rate': 0.02, 'random_state': 10}
X_train = df.drop(columns=['time33', 'time33_lag1', 'time33_lag2', 'time33_lag3', 'time33_lag4', 'time33_lag5', 'time33_lag6', 'time33_lag7'], axis=1)
y_train = df['time33']
model_xgb = xgb.train(params, dtrain=xgb.DMatrix(np.array(X_train), np.array(y_train)))
y_pred = model_xgb.predict(xgb.DMatrix(np.array(X_train)))
from sklearn.metrics import mean_squared_error
rmse = np.sqrt(mean_squared_error(y_pred, y_train))
print(rmse)
df.columns
X_train2 = df.drop(columns=['time0', 'time0_lag1', 'time0_lag2', 'time0_lag3', 'time0_lag4', 'time0_lag5', 'time0_lag6', 'time0_lag7'], axis=1)
y_pred2 = model_xgb.predict(xgb.DMatrix(np.array(X_train2))).clip(0, 20)
submission = pd.DataFrame({'ID': ID, 'item_cnt_month': y_pred2})
