...
def draw_box_plots(df, variables, n_rows, n_cols):
    ...
    for (i, var_name) in enumerate(variables):
        ax = fig.add_subplot(n_rows, n_cols, i + 1)
        sns.boxplot(x=df[var_name], ax=ax, color='purple')
...
plt.title('Correltaion plot between variables')
plt.xticks(rotation=60)

no_diab = data[data['Outcome'] == 0]
diab = data[data['Outcome'] == 1]
sns.heatmap(no_diab.corr(), annot=True, cmap='plasma')
sns.heatmap(diab.corr(), annot=True, cmap='magma')
...
plt.title('BMI distribution of diabetic and Diabetic')
...
plt.legend(['No Diabetic', 'Diabetic'])
plt.title('BloodPressure distribution of diabetic and Diabetic')
...
plt.legend(['No Diabetic', 'Diabetic'])
plt.title('Pregnancies distribution of diabetic and Diabetic')
data.columns
data.head()
data.columns
